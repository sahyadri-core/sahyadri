...
pub fn generate_stark_proof(batch: &mut DilithiumBatch) -> Result<BatchProof, ZKPError> {
    if batch.is_empty() { return Err(ZKPError::EmptyBatch); }
    ...
    for (i, att) in batch.attestations.iter().enumerate() {
        ...
        if !w.valid { return Err(ZKPError::VerificationFailed { index: i }); }
        let sp = prove_signature(&w)
            .map_err(|e| ZKPError::ProofGenerationFailed(format!("prove sig {} failed: {}", i, e)))?;
        sig_proofs.push(sp);
    }
    let commitment_root = batch.commitment_root();
    let proof_bytes = bincode::serialize(&sig_proofs)
        .map_err(|e| ZKPError::ProofSerializationFailed(e.to_string()))?;
    let prove_ms = start.elapsed().as_millis() as u64;
    let raw_bytes: usize = batch.attestations.iter()
    ...
    Ok(BatchProof {
        proof_bytes, commitment_root, batch_size: batch_size as u32,
        version: crate::PROOF_FORMAT_VERSION, generation_time_ms: prove_ms,
        stats: ProverStats { batch_size, raw_signature_bytes: raw_bytes,
            compressed_proof_bytes: proof_len, compression_ratio: ratio,
            prove_time_ms: prove_ms, verify_time_ms: 0, trace_rows: 0, trace_cols: 0 },
    })
}
...
pub fn prove_batch(messages: &[Vec<u8>], signatures: &[Vec<u8>], public_keys: &[Vec<u8>]) -> Result<BatchProof, ZKPError> {
    ...
    for i in 0..messages.len() {
        batch.add_and_verify(&messages[i],&signatures[i],&public_keys[i])
            .map_err(|e|{log::error!("[STARK-PROVER] sig {} failed: {}",i,e);e})?;
    }
