//! NTT (Number Theoretic Transform) for Dilithium.
//!
//! Faithful port of `ntt.c` from the CRYSTALS-Dilithium reference.
//! Operates on polynomial coefficients modulo Q = 8380417.

use crate::params::N;
use crate::reduce::montgomery_reduce;

/// Precomputed twiddle factors for the NTT.
/// These are powers of the root of unity in Montgomery form.
...
mod tests {
    ...
    fn test_ntt_invntt_roundtrip() {
        // Start with a known polynomial, apply NTT then INTT, verify recovery.
        let mut a = [0i32; N];
        for i in 0..N {
        ...
        // After NTT + INTT, each coefficient should be:
        // original[i] * 2^32 mod Q (Montgomery form)
        // But due to the scaling by f = 41978 = mont^2/256,
        // we get: a[i] ≡ original[i] * 2^32 * 2^{-32} / 256 * mont^2
