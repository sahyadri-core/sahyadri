...
impl<'a, T: VerifiableTransaction, Reused: SigHashReusedValues> TxScriptEngine<'a, T, Reused> {
    ...
    fn execute_opcode(&mut self, opcode: DynOpcodeImplementation<T, Reused>) -> Result<(), TxScriptError> {
        // Different from sahyadrid: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
    ...
    fn op_check_multisig(&mut self, _ecdsa: bool) -> Result<(), TxScriptError> {
        ...
        'outer: for (sig_idx, signature) in signatures.iter().enumerate() {
            ...
            loop {
                ...
                let check_signature_result = self.check_dilithium_signature_legacy(hash_type, pub_key.as_slice(), signature);
            ...
            ScriptSource::TxInput { tx, idx, .. } => {
                ...
                match self.sig_cache.get(&sig_cache_key) {
                    ...
                    None => {
                        ...
                        let valid = sahyadri_dilithium::DilithiumKeyPair::verify(
                            pk_bytes,
                            &sig,
                            &msg_bytes,
                            b"",
