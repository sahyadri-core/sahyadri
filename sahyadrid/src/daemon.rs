...
use sahyadri_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use sahyadri_core::{core::Core, debug, info};
use sahyadri_core::{sahyadrid_env::version, task::tick::TickService};
...
/// the application folder that contains sahyadrid logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                sahyadri_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, sahyadri_utils::sync::semaphore_module_path()));
            } else {
                sahyadri_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the Sahyadrid command line to confirm all interactive questions)";
    ...
    let mut meta_db = sahyadri_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = sahyadri_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from sahyadrid DB version
...
Do you confirm? (y/n)";
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = sahyadri_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", sahyadri_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", sahyadri_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", sahyadri_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
