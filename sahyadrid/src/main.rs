extern crate sahyadri_consensus;
extern crate sahyadri_core;
extern crate sahyadri_hashes;

use std::sync::Arc;

use sahyadri_alloc::init_allocator_with_default_settings;
use sahyadri_core::{info, signals::Signals};
...
pub fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("sahyadrid-heap.json").build();
    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The sahyadrid node requires a setting of at least {DESIRED_DAEMON_SOFT_FD_LIMIT} to operate properly.");
