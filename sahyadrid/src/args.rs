...
use sahyadri_core::sahyadrid_env::version;
use sahyadri_notify::address::tracker::Tracker;
use sahyadri_utils::networking::ContextualNetAddress;
use sahyadri_wrpc_server::address::WrpcNetAddress;
...
pub fn cli() -> Command {
    ...
    let cmd = Command::new("sahyadrid")
        .about(format!("{} (rusty-sahyadri) v{}", env!("CARGO_PKG_DESCRIPTION"), version()))
        .version(env!("CARGO_PKG_VERSION"))
        .arg(arg!(-C --configfile <CONFIG_FILE> "Path of config file.").env("SAHYADRID_CONFIGFILE"))
        .arg(arg!(-b --appdir <DATA_DIR> "Directory to store data.").env("SAHYADRID_APPDIR"))
        .arg(arg!(--logdir <LOG_DIR> "Directory to log output.").env("SAHYADRID_LOGDIR"))
        .arg(arg!(--nologfiles "Disable logging to files.").env("SAHYADRID_NOLOGFILES"))
        .arg(
            Arg::new("async_threads")
                .short('t')
                .long("async-threads")
                .env("SAHYADRID_ASYNC_THREADS")
        ...
        .arg(
            Arg::new("rpclisten")
                .long("rpclisten")
                .env("SAHYADRID_RPCLISTEN")
                .value_name("IP[:PORT]")
                .num_args(0..=1)
                .require_equals(true)
                .value_parser(clap::value_parser!(ContextualNetAddress))
                .help("Interface:port to listen for gRPC connections (default port: 26110, testnet: 16210)."),
        )
        .arg(
            Arg::new("rpclisten-borsh")
                .long("rpclisten-borsh")
                .env("SAHYADRID_RPCLISTEN_BORSH")
                .value_name("IP[:PORT]")
                .num_args(0..=1)
                .require_equals(true)
        ...
        .arg(arg!(--unsaferpc "Enable RPC commands which affect the state of the node").env("SAHYADRID_UNSAFERPC"))
        .arg(
            Arg::new("connect-peers")
                .long("connect")
                .env("SAHYADRID_CONNECTPEERS")
                .value_name("IP[:PORT]")
        ...
        .arg(
            Arg::new("listen")
                .long("listen")
                .env("SAHYADRID_LISTEN")
                .value_name("IP[:PORT]")
                .require_equals(true)
                .value_parser(clap::value_parser!(ContextualNetAddress))
                .help("Add an interface:port to listen for connections (default all interfaces port: 26111, testnet: 16211)."),
        )
        .arg(
            Arg::new("outpeers")
                .long("outpeers")
                .env("SAHYADRID_OUTPEERS")
                .value_name("outpeers")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
        ...
        .arg(
            Arg::new("enable-mainnet-mining")
                ...
                .env("SAHYADRID_ENABLE_MAINNET_MINING")
...
0, Tracker::MAX_ADDRESS_UPPER_BOUND, Tracker::DEFAULT_MAX_ADDRESSES)),
        ...
        .arg(arg!(--testnet "Use the test network").env("SAHYADRID_TESTNET"))
        .arg(
            Arg::new("netsuffix")
                .long("netsuffix")
                .env("SAHYADRID_NETSUFFIX")
                .value_name("netsuffix")
                .require_equals(true)
                .value_parser(clap::value_parser!(u32))
                .help("Testnet network suffix number"),
        )
        .arg(arg!(--devnet "Use the development test network").env("SAHYADRID_DEVNET"))
        .arg(arg!(--simnet "Use the simulation test network").env("SAHYADRID_SIMNET"))
        .arg(arg!(--archival "Run as an archival node: avoids deleting old block data when moving the pruning point (Warning: heavy disk usage)").env("SAHYADRID_ARCHIVAL"))
        .arg(arg!(--sanity "Enable various sanity checks which might be compute-intensive (mostly performed during pruning)").env("SAHYADRID_SANITY"))
        .arg(arg!(--yes "Answer yes to all interactive console questions").env("SAHYADRID_NONINTERACTIVE"))
        .arg(
            Arg::new("user_agent_comments")
                .long("uacomment")
                .env("SAHYADRID_USER_AGENT_COMMENTS")
                .action(ArgAction::Append)
                .require_equals(true)
        ...
        .arg(arg!(--"perf-metrics" "Enable performance metrics: cpu, memory, disk io usage").env("SAHYADRID_PERF_METRICS"))
        .arg(
            Arg::new("perf-metrics-interval-sec")
                .long("perf-metrics-interval-sec")
                .env("SAHYADRID_PERF_METRICS_INTERVAL_SEC")
                .require_equals(true)
                .value_parser(clap::value_parser!(u64))
                .help("Interval in seconds for performance metrics collection."),
        )
        .arg(arg!(--"disable-upnp" "Disable upnp").env("SAHYADRID_DISABLE_UPNP"))
        .arg(arg!(--"nodnsseed" "Disable DNS seeding for peers").env("SAHYADRID_NODNSSEED"))
        .arg(arg!(--"nogrpc" "Disable gRPC server").env("SAHYADRID_NOGRPC"))
        .arg(
            Arg::new("ram-scale")
                .long("ram-scale")
                .env("SAHYADRID_RAM_SCALE")
                .require_equals(true)
...
/*
  ...
  -C, --configfile=                         Path to configuration file (default: /Users/aspect/Library/Application
                                            Support/Sahyadrid/sahyadrid.conf)
  ...
  -a, --addpeer=                            Add a peer to connect with at startup
      ...
      --rpccert=                            File containing the certificate file (default:
                                            /Users/aspect/Library/Application Support/Sahyadrid/rpc.cert)
      --rpckey=                             File containing the certificate key (default:
                                            /Users/aspect/Library/Application Support/Sahyadrid/rpc.key)
  ...
  -d, --loglevel=                           Logging level for all subsystems {trace, debug, info, warn, error,
      ...
      --minrelaytxfee=                      The minimum transaction fee in CSM/kB to be considered a non-zero fee.
                                            (default: 1e-05)
