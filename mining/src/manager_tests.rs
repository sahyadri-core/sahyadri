...
mod tests {
    ...
    use sahyadri_mining_errors::mempool::RuleResult;
    ...
    fn test_replace_by_fee_in_mempool() {
        ...
        impl TxOp {
            fn change(&self) -> Option<u64> {
                self.change.then_some(900 * KANA_PER_SAHYADRI)
            }
        }
    ...
    fn test_revalidate_high_priority_transactions() {
        ...
        let (parent_tx, child_tx_1) = create_parent_and_children_transactions(&consensus, vec![3000 * KANA_PER_SAHYADRI]);
    ...
    fn generate_new_coinbase(address_prefix: Prefix, op: OpType) -> MinerData {
        match op {
            OpType::Usual => get_miner_data(address_prefix), // TODO: use lib_sahyadri_wallet.CreateKeyPair, util.NewAddressPublicKeyECDSA equivalents
            OpType::Edcsa => get_miner_data(address_prefix), // TODO: use lib_sahyadri_wallet.CreateKeyPair, util.NewAddressPublicKey equivalents
            OpType::True => {
    ...
    fn create_arrays_of_parent_and_children_transactions(
        ...
        (0..count)
            .map(|i| {
                create_parent_and_children_transactions(consensus, vec![500 * KANA_PER_SAHYADRI, 3_000 * KANA_PER_SAHYADRI + i as u64])
            })
            .unzip()
    }
    ...
    fn create_child_and_parent_txs_and_add_parent_to_consensus(consensus: &Arc<ConsensusMock>) -> Transaction {
        let parent_tx = create_transaction_without_input(vec![500 * KANA_PER_SAHYADRI]);
