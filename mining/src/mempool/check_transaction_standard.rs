...
use sahyadri_consensus_core::{hashing::sighash::SigHashReusedValuesUnsync, mass::NonContextualMasses};
use sahyadri_txscript::{get_sig_op_count_upper_bound, is_unspendable, script_class::ScriptClass};
...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of Sahyadri the consensus-valid transaction version range might diverge from the
    ...
    pub(crate) fn is_transaction_output_dust(&self, transaction_output: &TransactionOutput) -> bool {
        ...
        // mp.config.MinimumRelayTransactionFee is in kana/KB, so multiply
        // by 1000 to convert to bytes.
        //
        // Using the typical values for a pay-to-pubkey transaction from
        // the breakdown above and the default minimum free transaction relay
        // fee of 1000, this equates to values less than 546 kana being
        // considered dust.
        //
...
mod tests {
    ...
    fn test_is_transaction_output_dust() {
        ...
        let tests = vec![
            ...
            Test {
                name: "max kana amount is never dust",
                tx_out: TransactionOutput::new(MAX_KANA, script_public_key.clone()),
                minimum_relay_transaction_fee: 1000,
                is_dust: false,
            },
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_script_public_key = sahyadri_txscript::pay_to_address_script(&addr);
        let dummy_tx_out = TransactionOutput::new(KANA_PER_SAHYADRI, dummy_script_public_key);
