...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/sahyadrinet/sahyadrid/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        prefix: sahyadri_addresses::Prefix,
        ...
        {
            let script_public_key = sahyadri_txscript::pay_to_address_script(&sahyadri_addresses::Address::new(
                prefix,
            ...
            let BlockTemplate { block: sahyadri_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use sahyadri_consensus_core::subnets;
