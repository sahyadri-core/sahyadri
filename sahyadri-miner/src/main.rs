...
async fn main() -> Result<(), Error> {
    ...
    while !shutdown.is_shutdown() {
        let mut client =
            SahyadridHandler::connect(opt.sahyadrid_address.clone(), opt.mining_address.clone(), opt.mine_when_not_synced)
                .await?;
        ...
        warn!("Disconnected from sahyadrid, retrying");
    }
    Ok(())
}
