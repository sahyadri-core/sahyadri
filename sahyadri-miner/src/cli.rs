...
pub struct Opt {
    ...
    /// The Sahyadri address for the miner reward
    pub mining_address: String,
    #[clap(short = 's', long = "sahyadrid-address", default_value = "127.0.0.1", display_order = 1)]
    /// The IP of the sahyadrid instance
    pub sahyadrid_address: String,

    #[clap(long = "devfund", display_order = 6)]
    /// Mine a percentage of the blocks to the Sahyadri devfund [default: Off]
    ...
    /// Sahyadrid port [default: Mainnet = 26110, Testnet = 16210]
    port: Option<u16>,
    ...
    /// Mine even when sahyadrid says it is not synced, only useful when passing `--allow-submit-block-when-not-synced` to sahyadrid  [default: false]
    ...
    /// Output logs in alternative format (same as sahyadrid)
    pub altlogs: bool,
}
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        if self.sahyadrid_address.is_empty() {
            self.sahyadrid_address = "127.0.0.1".to_string();
        }

        if !self.sahyadrid_address.starts_with("grpc://") {
            IpAddr::from_str(&self.sahyadrid_address)?;
            let port = self.port();
            self.sahyadrid_address = format!("grpc://{}:{}", self.sahyadrid_address, port);
        }
        log::info!("Sahyadrid address: {}", self.sahyadrid_address);

        Ok(())
    }

    fn port(&mut self) -> u16 {
        *self.port.get_or_insert(if self.testnet { 16210 } else { 26110 })
    }
