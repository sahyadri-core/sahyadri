...
The `--override-params-file` flag lets you run `sahyadrid` with a custom set of
...
3. Launch `sahyadrid` with the flag:

	 ```bash
	 sahyadrid --devnet --override-params-file /path/to/overrides.json
	 ```

If the file cannot be read or parsed, `sahyadrid` prints the error and exits.
...
| sahyadri_consensus_k                          | SahyadriConsensus K                          |
...
```bash
sahyadrid --simnet --override-params-file overrides.json
```