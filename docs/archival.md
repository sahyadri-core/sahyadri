# Running Sahyadri Archive Nodes

This guide explains how to run a Sahyadri archive node with HDD-optimized RocksDB configuration.
...
Normal Sahyadri nodes are **pruned** and only keep recent blocks (determined by pruning depth).
...
Sahyadrid provides two RocksDB configuration presets optimized for different storage types:

### Default Preset (SSD/NVMe)
```bash
sahyadrid --archival
# or explicitly:
sahyadrid --archival --rocksdb-preset=default
```
...
### HDD Preset
```bash
sahyadrid --archival --rocksdb-preset=hdd
```
...
```bash
# /etc/sysctl.d/90-sahyadrid-archive.conf
vm.dirty_ratio = 40
...
Apply with: `sudo sysctl -p /etc/sysctl.d/90-sahyadrid-archive.conf`
...
sahyadrid --archival --rocksdb-preset=hdd --ram-scale=0.2

# Normal RAM (8GB system)
sahyadrid --archival --rocksdb-preset=hdd --ram-scale=0.5

# High RAM (16GB+ system)
sahyadrid --archival --rocksdb-preset=hdd --ram-scale=1.0
```
...
sahyadrid --archival --rocksdb-preset=hdd --rocksdb-cache-size=2048  # 2GB cache
```
...
```bash
# Using sahyadri-cli (if installed)
sahyadri-cli getinfo

# Check logs
journalctl -u sahyadrid -f

# Check disk usage
du -sh ~/.sahyadri/sahyadri-mainnet/datadir/
```

### Performance Metrics
```bash
# Enable performance metrics
sahyadrid --archival --rocksdb-preset=hdd --perf-metrics --perf-metrics-interval-sec=60
```
...
services:
  sahyadrid-archive:
    image: sahyadrinet/sahyadrid:latest
    container_name: sahyadrid-archive
...
# Run container
docker run -d \
  --name sahyadrid-archive \
  --restart unless-stopped \
  -v /mnt/hdd/sahyadri-archive:/app/data \
  -p 16111:16111 \
  -p 17110:17110 \
  -p 18110:18110 \
  sahyadrinet/sahyadrid:latest \
    --archival \
    --rocksdb-preset=hdd \
    --ram-scale=1.0 \
    --rpclisten-borsh=0.0.0.0:17110 \
...
**/etc/systemd/system/sahyadrid-archive.service:**
```ini
[Unit]
Description=Sahyadri Archive Node (HDD-optimized)
...
User=sahyadri
Group=sahyadri
ExecStart=/usr/local/bin/sahyadrid \
  --archival \
  --rocksdb-preset=hdd \
  --ram-scale=1.0 \
...
sudo systemctl enable sahyadrid-archive
sudo systemctl start sahyadrid-archive
sudo systemctl status sahyadrid-archive
```
...
1. Verify HDD preset is active:
   ```bash
   journalctl -u sahyadrid-archive | grep "RocksDB preset"
...
2. Reduce `--ram-scale` and cache size:
   ...
   sahyadrid --archival --rocksdb-preset=hdd --rocksdb-cache-size=64
   ```
...
systemctl cat sahyadrid-archive | grep rocksdb-preset

# Check startup logs
journalctl -u sahyadrid-archive -n 100 | grep -i rocksdb
