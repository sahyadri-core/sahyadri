...
use super::sahyadri_consensus::ordering::SortableBlock;
...
trait DifficultyManagerExtension {
    ...
    #[inline]
    #[must_use]
    fn internal_calc_daa_score(&self, sahyadri_consensus_data: &SahyadriConsensusData, mergeset_non_daa: &BlockHashSet) -> u64 {
        let sp_daa_score = self.headers_store().get_daa_score(sahyadri_consensus_data.selected_parent).unwrap();
        sp_daa_score + (sahyadri_consensus_data.mergeset_size() - mergeset_non_daa.len()) as u64
    }
...
/// A difficulty manager based on sampled block windows, implementing [KIP-0004](https://github.com/sahyadrinet/kips/blob/master/kip-0004.md)
#[derive(Clone)]
pub struct SampledDifficultyManager<T: HeaderStoreReader, U: SahyadriConsensusStoreReader> {
    headers_store: Arc<T>,
    _sahyadri_consensus_store: Arc<U>,
    genesis_hash: Hash,
    genesis_bits: u32,
...
impl<T: HeaderStoreReader, U: SahyadriConsensusStoreReader> SampledDifficultyManager<T, U> {
    pub fn new(
        headers_store: Arc<T>,
        sahyadri_consensus_store: Arc<U>,
        genesis_hash: Hash,
        genesis_bits: u32,
        ...
        Self {
            headers_store,
            _sahyadri_consensus_store: sahyadri_consensus_store,
            genesis_hash,
            genesis_bits,
    ...
    #[inline]
    #[must_use]
    pub fn lowest_daa_blue_score(&self, sahyadri_consensus_data: &SahyadriConsensusData) -> u64 {
        let difficulty_full_window_size = self.difficulty_full_window_size();
        sahyadri_consensus_data.blue_score.max(difficulty_full_window_size) - difficulty_full_window_size
    }

    #[inline]
    #[must_use]
    pub fn calc_daa_score(&self, sahyadri_consensus_data: &SahyadriConsensusData, mergeset_non_daa: &BlockHashSet) -> u64 {
        self.internal_calc_daa_score(sahyadri_consensus_data, mergeset_non_daa)
    }
    ...
    pub fn calculate_difficulty_bits(&self, window: &BlockWindowHeap, sahyadri_consensus_data: &SahyadriConsensusData) -> u32 {
        ...
        if difficulty_blocks.len() < self.min_difficulty_window_size {
            let selected_parent = sahyadri_consensus_data.selected_parent;
...
impl<T: HeaderStoreReader, U: SahyadriConsensusStoreReader> DifficultyManagerExtension for SampledDifficultyManager<T, U> {
    fn headers_store(&self) -> &dyn HeaderStoreReader {
...
#[cfg(test)]
mod tests {
    use sahyadri_consensus_core::{BlockLevel, BlueWorkType, MAX_WORK_LEVEL};
    use sahyadri_math::{Uint256, Uint320};
    use sahyadri_pow::calc_level_from_pow;
