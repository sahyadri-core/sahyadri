...
impl<T: SahyadriConsensusStoreReader, S: RelationsStoreReader, U: ReachabilityService, V: HeaderStoreReader> SahyadriConsensusManager<T, S, U, V> {
    pub fn new(
        ...
        Self { genesis_hash, k, sahyadri_consensus_store, relations_store, reachability_service, headers_store, level_work: 0.into() }
    }
    ...
    pub fn genesis_sahyadri_consensus_data(&self) -> SahyadriConsensusData {
        SahyadriConsensusData::new(
            0,
            Default::default(),
            blockhash::ORIGIN,
    ...
    pub fn origin_sahyadri_consensus_data(&self) -> Arc<SahyadriConsensusData> {
        Arc::new(SahyadriConsensusData::new(
            0,
            Default::default(),
            0.into(),
            BlockHashes::new(Vec::new()),
    ...
    pub fn find_selected_parent(&self, parents: impl IntoIterator<Item = Hash>) -> Hash {
        parents
            .into_iter()
            .map(|parent| SortableBlock { hash: parent, blue_work: self.sahyadri_consensus_store.get_blue_work(parent).unwrap() })
            .max()
            .unwrap()
            .hash
    }

    /// Runs the SAHYADRI_CONSENSUS protocol and calculates the block SahyadriConsensusData by the given parents.
    ...
    pub fn sahyadri_consensus(&self, parents: &[Hash]) -> SahyadriConsensusData {
        assert!(!parents.is_empty(), "genesis must be added via a call to init");

        // Run the SAHYADRI_CONSENSUS parent selection algorithm
        ...
        if selected_parent.is_origin() {
            ...
            return SahyadriConsensusData::new_with_selected_parent(selected_parent, 1); // k is only a capacity hint here
        }
        let k = self.k;
        // Initialize new SAHYADRI_CONSENSUS block data with the selected parent
        let mut new_block_data = SahyadriConsensusData::new_with_selected_parent(selected_parent, k);
        ...
        let added_blue_work: BlueWorkType = new_block_data
            ...
            .sum();
        let blue_work: BlueWorkType = self.sahyadri_consensus_store.get_blue_work(selected_parent).unwrap() + added_blue_work;
    ...
    fn check_blue_candidate_with_chain_block(
        &self,
        new_block_data: &SahyadriConsensusData,
    ...
    fn blue_anticone_size(&self, block: Hash, context: &SahyadriConsensusData) -> KType {
        ...
        loop {
            ...
            current_blues_anticone_sizes = self.sahyadri_consensus_store.get_blues_anticone_sizes(current_selected_parent).unwrap();
            current_selected_parent = self.sahyadri_consensus_store.get_selected_parent(current_selected_parent).unwrap();
        }
    }

    fn check_blue_candidate(&self, new_block_data: &SahyadriConsensusData, blue_candidate: Hash, k: KType) -> ColoringOutput {
        ...
        loop {
            ...
            chain_block = ChainBlock {
                ...
                data: self.sahyadri_consensus_store.get_data(chain_block.data.selected_parent).unwrap().into(),
            }
        }
    }
}

/// Chain block with attached sahyadri_consensus data
struct ChainBlock<'a> {
    hash: Option<Hash>, // if set to `None`, signals being the new block
    data: Refs<'a, SahyadriConsensusData>,
}

/// Represents the intermediate SAHYADRI_CONSENSUS coloring state for the current candidate
enum ColoringState {
    Blue,
    Red,
    Pending,
...
/// Represents the final output of SAHYADRI_CONSENSUS coloring for the current candidate
enum ColoringOutput {
