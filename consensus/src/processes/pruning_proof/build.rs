...
struct LevelProofContext {
    sahyadri_consensus_store: Arc<DbSahyadriConsensusStore>,
    tip: Hash,
    root: Hash,
    count: u64,
}
...
/// Utility for creating retry-indexed temporary sahyadri_consensus stores.
///
/// Each call to `new_store` returns a fresh temporary `DbSahyadriConsensusStore` for the
/// given level, using an incrementing retry index to avoid namespace collisions.
/// This is used when multiple sahyadri_consensus attempts may be required for the same level.
struct SahyadriConsensusStoreFactory {
    db: Arc<DB>,
...
impl SahyadriConsensusStoreFactory {
...
impl PruningProofManager {
    ...
    fn proof_from_descriptor(&self, descriptor: &PruningProofDescriptor) -> PruningPointProof {
        ...
        let (_db_lifetime, temp_db) = sahyadri_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
    ...
    fn calc_new_proof(&self, pp: Hash, prev_descriptor: Option<&PruningProofDescriptor>) -> PruningProofDescriptor {
        ...
        for level in (0..=self.max_block_level).rev() {
            ...
            let required_block = if level != self.max_block_level {
                let LevelProofContext { sahyadri_consensus_store: next_level_gd_store, tip: next_level_tip, .. } =
    ...
    ///    (e.g. future-size tracking, sahyadri_consensus inputs) is observed consistently.
    /// 3. Whenever the current block is a viable root candidate (sufficient base-level depth,
    ///    sufficient future size, and inclusion constraints), populate a temporary sahyadri_consensus store
    ...
    fn calc_level_proof_context(
        ...
        // the temporary sahyadri_consensus instance at this level.
        ...
        //   lacks sufficient blues here, we avoid premature sahyadri_consensus attempts.
        ...
        // Each sahyadri_consensus attempt uses a fresh temp store namespace (indexed internally by `retries`).
        let mut sahyadri_consensus_factory = SahyadriConsensusStoreFactory::new(db.clone(), cache_policy, level);
        ...
        let mut try_root = |relations_store: &DbRelationsStore, root: Hash, future_size: u64| -> Option<LevelProofContext> {
            // Populate sahyadri_consensus for `future(root) ∩ past(tip)` and test depth requirements.
            let (sahyadri_consensus_store, has_required_block, count) = self.populate_level_proof_sahyadri_consensus_data(
                relations_store,
                &mut sahyadri_consensus_factory,
                &mut reachability_factory,
                root,
                tip,
                required,
                level,
                self.sahyadri_consensus_k,
            );

            // Realized blue depth for this root, computed from the level-specific sahyadri_consensus
            let current_level_score = sahyadri_consensus_store.get_blue_score(tip).unwrap();

            // Log all non-trivial cases
            if tip != self.genesis_hash {
                debug!(
                    "level: {}, future: {}, blue score: {}, count: {}, retries: {}",
                    level, future_size, current_level_score, count, sahyadri_consensus_factory.retries
                );
            }
            ...
            if has_required_block && (root == self.genesis_hash || current_level_score >= 2 * self.pruning_proof_m) {
                Some(LevelProofContext { sahyadri_consensus_store, tip, root, count })
            } else {
                None
            }
        };
        ...
        while let Some(SortableBlock { hash: current, .. }) = queue.pop() {
            ...
            if self.reachability_service.is_dag_ancestor_of(current, required) {
                // If the root appears immediately viable, attempt sahyadri_consensus now.
                    ...
                    || (future_size >= required_future_size && base_level_depth >= required_base_level_depth)
                {
                    ...
                    // Defer sahyadri_consensus and keep as a fallback candidate.
        ...
        // With a fixed root, sahyadri_consensus selection is deterministic, and if the new tip is in the future of the
    ...
    /// Forward-traverses from `root` toward `tip`, and inserts sahyadri_consensus data for each visited block.
    ///
    ...
    fn populate_level_proof_sahyadri_consensus_data(
        &self,
        relations_store: &DbRelationsStore,
        sahyadri_consensus_factory: &mut SahyadriConsensusStoreFactory,
    ...
    ) -> (Arc<DbSahyadriConsensusStore>, bool, u64) {
        debug!("Populating GD for root {} at level {} (retry {})", root, level, sahyadri_consensus_factory.retries.saturating_sub(1));

        let sahyadri_consensus_store = sahyadri_consensus_factory.new_store();
        ...
        // Create a sahyadri_consensus manager over the restricted relations view
        let sahyadri_consensus_manager = SahyadriConsensusManager::with_level(
            root,
        ...
        sahyadri_consensus_store.insert(root, Arc::new(sahyadri_consensus_manager.genesis_sahyadri_consensus_data())).unwrap();
        ...
        while let Some(Reverse(SortableBlock { hash: current, .. })) = queue.pop() {
            ...
            let sahyadri_consensus_data = Arc::new(sahyadri_consensus_manager.sahyadri_consensus(parents.as_slice()));
            sahyadri_consensus_store.insert(current, sahyadri_consensus_data.clone()).unwrap();

            reachability_tip = sahyadri_consensus_manager.find_selected_parent([reachability_tip, current]);
        ...
        (sahyadri_consensus_store, has_required_block, count)
    }
