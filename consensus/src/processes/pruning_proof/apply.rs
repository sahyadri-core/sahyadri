...
impl PruningProofManager {
    pub fn apply_proof(&self, proof: PruningPointProof, trusted_set: &[TrustedBlock]) -> PruningImportResult<()> {
        ...
        let mut trusted_gd_map: BlockHashMap<SahyadriConsensusData> = BlockHashMap::new();
        // This loop expands the proof with the headers of the trusted set
        // and creates a hash to sahyadri_consensus data map of the trusted set
        for tb in trusted_set.iter() {
            trusted_gd_map.insert(tb.block.hash(), tb.sahyadri_consensus.clone().into());
        ...
        // Populate sahyadri_consensus_store and relation store for every block in the proof
        ...
        for header in expanded_proof[0].iter() {
            ...
            let gd = if let Some(gd) = trusted_gd_map.get(&header.hash) {
                ...
                let calculated_gd = self.sahyadri_consensus_manager.sahyadri_consensus(&parents);
                // Override the sahyadri_consensus data with the real blue score and blue work
                SahyadriConsensusData {
            ...
            self.sahyadri_consensus_store.insert(header.hash, Arc::new(gd)).unwrap();
