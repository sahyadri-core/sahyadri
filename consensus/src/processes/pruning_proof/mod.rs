...
pub struct PruningProofManager {
    ...
    sahyadri_consensus_manager: DbSahyadriConsensusManager,
...
impl PruningProofManager {
    ...
    ///
    /// The returned hashes are guaranteed to have SAHYADRI_CONSENSUS data
    pub(crate) fn get_sahyadri_consensus_chain_k_depth(&self, hash: Hash, sahyadri_consensus_k: KType) -> Vec<Hash> {
        let mut hashes = Vec::with_capacity(sahyadri_consensus_k as usize + 1);
        let mut current = hash;
        for _ in 0..=sahyadri_consensus_k {
            hashes.push(current);
            let Some(parent) = self.sahyadri_consensus_store.get_selected_parent(current).optional().unwrap() else {
                break;
            };
    ...
    pub(crate) fn calculate_pruning_point_anticone_and_trusted_data(
        ...
        let mut anticone = self.sahyadri_consensus_manager.sort_blocks(anticone);
        anticone.insert(0, pruning_point);

        let mut daa_window_blocks = BlockHashMap::new();
        let mut sahyadri_consensus_blocks = BlockHashMap::new();

        let sahyadri_consensus_k = self.sahyadri_consensus_k;
        ...
        for anticone_block in anticone.iter().copied() {
            let sahyadri_consensus = self.sahyadri_consensus_store.get_data(anticone_block).unwrap();
            let window = self.window_manager.block_window(&sahyadri_consensus, WindowType::DifficultyWindow).unwrap();
            let cover = self.window_manager.consecutive_cover_for_window(sahyadri_consensus, &window);

            for hash in cover {
                if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                    e.insert(TrustedHeader {
                        header: self.headers_store.get_header(hash).unwrap(),
                        sahyadri_consensus: (&*self.sahyadri_consensus_store.get_data(hash).unwrap()).into(),
                    });
                }
            }

            let sahyadri_consensus_chain = self.get_sahyadri_consensus_chain_k_depth(anticone_block, sahyadri_consensus_k);
            for hash in sahyadri_consensus_chain {
                if let Entry::Vacant(e) = sahyadri_consensus_blocks.entry(hash) {
                    let sahyadri_consensus = self.sahyadri_consensus_store.get_data(hash).unwrap();
                    e.insert((&*sahyadri_consensus).into());

                    // We fill `sahyadri_consensus_blocks` only for sahyadrid-go legacy reasons, but the real set we
                    // send is `daa_window_blocks` which represents the full trusted sub-DAG in the antifuture
                    // of the pruning point which sahyadrid-rust nodes expect to get when synced with headers proof
                    if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                        e.insert(TrustedHeader {
                            header: self.headers_store.get_header(hash).unwrap(),
                            sahyadri_consensus: (&*sahyadri_consensus).into(),
                        });
                    }
                }
            }
        ...
        while let Some(current) = queue.pop_front() {
            if let Entry::Vacant(e) = daa_window_blocks.entry(current) {
                ...
                let sahyadri_consensus = (&*self.sahyadri_consensus_store.get_data(current).unwrap()).into();
                e.insert(TrustedHeader { header, sahyadri_consensus });
            }
    ...
    pub fn get_pruning_point_anticone_and_trusted_data(&self) -> ConsensusResult<Arc<PruningPointTrustedData>> {
        ...
        if virtual_state.sahyadri_consensus_data.blue_score >= pp_bs + self.anticone_finalization_depth {
...
trait SahyadriConsensusReaderExt
where
    Self: SahyadriConsensusStoreReader,
{
...
impl<T: SahyadriConsensusStoreReader> SahyadriConsensusReaderExt for T {}
