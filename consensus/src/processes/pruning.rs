...
pub struct PruningPointManager<
    S: SahyadriConsensusStoreReader,
...
> {
    ...
    sahyadri_consensus_store: Arc<S>,
    headers_store: Arc<U>,
...
> PruningPointManager<S, T, U, V, W, Y>
{
    ...
    ///
    /// Let B denote the current block (represented by `sahyadri_consensus_data`)
    /// Assumptions:
    ...
    pub fn expected_header_pruning_point(&self, sahyadri_consensus_data: CompactSahyadriConsensusData) -> PruningPointReply {
        //
        // Note that past pruning samples are only assumed to have a header store entry and a pruning sample
        // store entry, se we only use these stores here (and specifically do not use the sahyadri_consensus store)
        //

        let pruning_depth = self.pruning_depth;
        let finality_depth = self.finality_depth;

        let selected_parent_blue_score = self.headers_store.get_blue_score(sahyadri_consensus_data.selected_parent).unwrap();

        let pruning_sample = if sahyadri_consensus_data.selected_parent == self.genesis_hash {
            self.genesis_hash
        } else {
            let selected_parent_pruning_sample =
                self.pruning_samples_store.pruning_sample_from_pov(sahyadri_consensus_data.selected_parent).unwrap();
        ...
        let pruning_point = loop {
            ...
            if current_blue_score + pruning_depth <= sahyadri_consensus_data.blue_score {
                break current;
            }
    ...
    pub fn next_pruning_points(&self, sink_sahyadri_consensus: CompactSahyadriConsensusData, current_pruning_point: Hash) -> Vec<Hash> {
        ...
        if current_pruning_point_blue_score + self.pruning_depth > sink_sahyadri_consensus.blue_score {
            // During initial IBD the sink can be close to the global pruning point.
            return vec![];
        }

        let sink_pruning_point = self.expected_header_pruning_point(sink_sahyadri_consensus).pruning_point;
    ...
    pub fn is_valid_pruning_point(&self, pp_candidate: Hash, tip: Hash) -> bool {
        ...
        let tip_bs = self.sahyadri_consensus_store.get_blue_score(tip).unwrap();
...
#[cfg(test)]
mod tests {
    use sahyadri_consensus_core::{config::params::Params, network::NetworkType};
    ...
    fn assert_pruning_depth_consistency() {
        for net in NetworkType::iter() {
            ...
            let sahyadri_consensus_k = params.sahyadri_consensus_k();

            // Assert P is not a multiple of F +- noise(K)
            let mod_after = pruning_depth % finality_depth;
            assert!((sahyadri_consensus_k as u64) < mod_after && mod_after < finality_depth - sahyadri_consensus_k as u64);
        }
    }
}
