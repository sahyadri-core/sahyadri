...
## Sequential processing flow (in go-sahyadrid)

Below we detail the current state of affairs in *go-sahyadrid* and discuss future parallelism opportunities. Processing dependencies between various stages are detailed in square brackets [***deps; type***].
...
* Pow:
    ...
    * check difficulty and blue work
        * run SAHYADRI_CONSENSUS and stage [***reachability; read*** | ***sahyadri_consensus; write***]
...
* Post pow (aka "*header in context*"):
    ...
    * check mergeset size limit (could be done following SAHYADRI_CONSENSUS)
...
The current code design (*go-sahyadrid*) already logically supports this since the various processing stages were already decoupled for supporting efficient IBD.
