...
pub struct HeaderProcessingContext {
    ...
    // Staging data
    pub sahyadri_consensus_data: Option<Arc<SahyadriConsensusData>>,
...
impl HeaderProcessingContext {
    ...
    /// Returns the primary (level 0) SAHYADRI_CONSENSUS data of this header.
    /// NOTE: expected to be called only after SAHYADRI_CONSENSUS computation was pushed into the context
    pub fn sahyadri_consensus_data(&self) -> &Arc<SahyadriConsensusData> {
        self.sahyadri_consensus_data.as_ref().unwrap()
    }
}
...
impl HeaderProcessor {
    ...
    fn validate_header(&self, header: &Arc<Header>) -> BlockProcessResult<HeaderProcessingContext> {
        ...
        self.sahyadri_consensus(&mut ctx);
    ...
    // Runs partial header validation for trusted blocks (currently validates only header-in-isolation and computes SAHYADRI_CONSENSUS).
    ...
    /// Runs the SAHYADRI_CONSENSUS algorithm and writes the data into the context (if hasn't run already)
    fn sahyadri_consensus(&self, ctx: &mut HeaderProcessingContext) {
        let sahyadri_consensus_data = self
            .sahyadri_consensus_store
            .get_data(ctx.hash)
            .optional()
            .unwrap()
            .unwrap_or_else(|| Arc::new(self.sahyadri_consensus_manager.sahyadri_consensus(&ctx.known_direct_parents)));
        self.counters.mergeset_counts.fetch_add(sahyadri_consensus_data.mergeset_size() as u64, Ordering::Relaxed);
        ctx.sahyadri_consensus_data = Some(sahyadri_consensus_data);
    }

    fn commit_header(&self, ctx: HeaderProcessingContext, header: &Header) {
        let sahyadri_consensus_data = ctx.sahyadri_consensus_data.as_ref().unwrap();
        ...
        //
        self.sahyadri_consensus_store.insert_batch(&mut batch, ctx.hash, sahyadri_consensus_data).unwrap();
        ...
        let mut reachability_mergeset = sahyadri_consensus_data.unordered_mergeset_without_selected_parent();
    ...
    fn commit_trusted_header(&self, ctx: HeaderProcessingContext, _header: &Header) {
        ...
        self.sahyadri_consensus_store.insert_batch(&mut batch, ctx.hash, sahyadri_consensus_data).idempotent().unwrap();
    ...
    pub fn process_genesis(&self) {
        ...
        ctx.sahyadri_consensus_data = Some(Arc::new(self.sahyadri_consensus_manager.genesis_sahyadri_consensus_data()));
