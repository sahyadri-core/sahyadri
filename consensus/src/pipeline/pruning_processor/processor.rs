...
pub enum PruningProcessingMessage {
    Exit,
    Process { sink_sahyadri_consensus_data: CompactSahyadriConsensusData },
}
...
impl PruningProcessor {
    ...
    pub fn worker(self: &Arc<Self>) {
        ...
        while let Ok(PruningProcessingMessage::Process { sink_sahyadri_consensus_data }) = self.receiver.recv() {
            if !recovered {
            ...
            self.advance_pruning_point_if_possible(sink_sahyadri_consensus_data);
        }
    }
    ...
    fn advance_pruning_point_if_possible(&self, sink_sahyadri_consensus_data: CompactSahyadriConsensusData) {
        let pruning_point_read = self.pruning_point_store.upgradable_read();
        let (current_pruning_point, current_index) = pruning_point_read.pruning_point_and_index().unwrap();
        let new_pruning_points = self.pruning_point_manager.next_pruning_points(sink_sahyadri_consensus_data, current_pruning_point);
    ...
    fn prune(&self, new_pruning_point: Hash, retention_period_root: Hash) {
        ...
        let mut keep_relations: BlockHashMap<BlockLevel> = std::iter::empty()
            ...
            .chain(data.sahyadri_consensus_blocks.iter().map(|gd| gd.hash))
...

        {
            let mut counter = 0;
            let mut batch = WriteBatch::default();
            // At this point keep_relations only holds level-0 relations which is the correct filtering criteria for primary SAHYADRI_CONSENSUS
            for kept in keep_relations.keys().copied() {
                let Some(sahyadri_consensus) = self.sahyadri_consensus_store.get_data(kept).optional().unwrap() else {
                    continue;
                };
                if sahyadri_consensus.unordered_mergeset().any(|h| !keep_relations.contains_key(&h)) {
                    let mut mutable_sahyadri_consensus: ExternalSahyadriConsensusData = sahyadri_consensus.as_ref().into();
                    mutable_sahyadri_consensus.mergeset_blues.retain(|h| keep_relations.contains_key(h));
                    mutable_sahyadri_consensus.mergeset_reds.retain(|h| keep_relations.contains_key(h));
                    mutable_sahyadri_consensus.blues_anticone_sizes.retain(|k, _| keep_relations.contains_key(k));
                    if !keep_relations.contains_key(&mutable_sahyadri_consensus.selected_parent) {
            ...
            self.db.write(batch).unwrap();
            info!("Header and Block pruning: updated sahyadri_consensus data for {} blocks", counter);
        }

        // No need to hold the prune guard while we continue populating keep_relations
        drop(prune_guard);

        // Add additional levels only after filtering SAHYADRI_CONSENSUS data via level 0
        ...
        while let Some(current) = queue.pop_front() {
            ...
            if !keep_blocks.contains(&current) {
                ...
                if let Some(&affiliated_proof_level) = keep_relations.get(&current) {
                    ...
                    // delete relations and sahyadri_consensus unless current is in level 0 of the pruning proof
                    if affiliated_proof_level > 0 {
                        ...
                        self.sahyadri_consensus_store.delete_batch(&mut batch, current).optional().unwrap();
                    }
                    ...
                    counter += 1;
                    // Prune data related to headers: relations, reachability, sahyadri_consensus
    ...
    fn get_sink(&self) -> Hash {
        self.lkg_virtual_state.load().sahyadri_consensus_data.selected_parent
    }
    ...
    fn confirm_pruning_depth_below_virtual(&self, pruning_point: Hash) -> bool {
        ...
        virtual_state.sahyadri_consensus_data.blue_score >= pp_bs + self.config.params.pruning_depth()
    }
    ...
    fn assert_data_rebuilding(&self, ref_data: Arc<PruningPointTrustedData>, new_pruning_point: Hash) {
        ...
        assert_eq!(
            ref_data.sahyadri_consensus_blocks.iter().map(|gd| gd.hash).collect::<BlockHashSet>(),
            built_data.sahyadri_consensus_blocks.iter().map(|gd| gd.hash).collect::<BlockHashSet>()
        );
