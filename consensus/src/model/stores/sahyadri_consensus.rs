...
use sahyadri_consensus_core::trusted::ExternalSahyadriConsensusData;
use sahyadri_consensus_core::{BlockHashMap, BlockHasher, BlockLevel, HashMapCustomHasher};
use sahyadri_consensus_core::{BlueWorkType, blockhash::BlockHashes};
use sahyadri_database::prelude::DB;
use sahyadri_database::prelude::{BatchDbWriter, CachedDbAccess, DbKey};
use sahyadri_database::prelude::{CachePolicy, StoreError};
...
pub use sahyadri_consensus_core::{HashKTypeMap, KType};

#[derive(Clone, Serialize, Deserialize, Default)]
pub struct SahyadriConsensusData {
    pub blue_score: u64,
...
pub struct CompactSahyadriConsensusData {
    pub blue_score: u64,
    pub blue_work: BlueWorkType,
    pub selected_parent: Hash,
}

impl MemSizeEstimator for SahyadriConsensusData {
...
impl MemSizeEstimator for CompactSahyadriConsensusData {}

impl From<&SahyadriConsensusData> for CompactSahyadriConsensusData {
    #[inline(always)]
    fn from(value: &SahyadriConsensusData) -> Self {
        Self { blue_score: value.blue_score, blue_work: value.blue_work, selected_parent: value.selected_parent }
    }
}

impl From<ExternalSahyadriConsensusData> for SahyadriConsensusData {
    fn from(value: ExternalSahyadriConsensusData) -> Self {
        Self {
...
impl From<&SahyadriConsensusData> for ExternalSahyadriConsensusData {
    fn from(value: &SahyadriConsensusData) -> Self {
        Self {
            blue_score: value.blue_score,
            blue_work: value.blue_work,
...
impl SahyadriConsensusData {
    ...
    pub fn ascending_mergeset_without_selected_parent<'a>(
        &'a self,
        store: &'a (impl SahyadriConsensusStoreReader + ?Sized),
    ...
    pub fn to_compact(&self) -> CompactSahyadriConsensusData {
        self.into()
    }
...
pub trait SahyadriConsensusStoreReader {
    ...
    fn get_data(&self, hash: Hash) -> Result<Arc<SahyadriConsensusData>, StoreError>;

    fn get_compact_data(&self, hash: Hash) -> Result<CompactSahyadriConsensusData, StoreError>;

    /// Check if the store contains data for the requested hash
    fn has(&self, hash: Hash) -> Result<bool, StoreError>;
}

pub trait SahyadriConsensusStore: SahyadriConsensusStoreReader {
    /// Insert SAHYADRI_CONSENSUS data for block `hash` into the store. Note that SAHYADRI_CONSENSUS data
    ...
    fn insert(&self, hash: Hash, data: Arc<SahyadriConsensusData>) -> Result<(), StoreError>;
    fn delete(&self, hash: Hash) -> Result<(), StoreError>;
}

/// A DB + cache implementation of `SahyadriConsensusStore` trait, with concurrency support.
#[derive(Clone)]
pub struct DbSahyadriConsensusStore {
    db: Arc<DB>,
    level: BlockLevel,
    access: CachedDbAccess<Hash, Arc<SahyadriConsensusData>, BlockHasher>,
    compact_access: CachedDbAccess<Hash, CompactSahyadriConsensusData, BlockHasher>,
}

impl DbSahyadriConsensusStore {
    pub fn new(db: Arc<DB>, level: BlockLevel, cache_policy: CachePolicy, compact_cache_policy: CachePolicy) -> Self {
        ...
        let prefix = DatabaseStorePrefixes::SahyadriConsensus.into_iter().chain(lvl_bytes).collect_vec();
        let compact_prefix = DatabaseStorePrefixes::SahyadriConsensusCompact.into_iter().chain(lvl_bytes).collect_vec();
        Self {
            db: Arc::clone(&db),
            level,
    ...
    pub fn new_temp(
        ...
        let prefix = DatabaseStorePrefixes::TempSahyadriConsensus.into_iter().chain(lvl_bytes).chain(temp_index_bytes).collect_vec();
        let compact_prefix =
            DatabaseStorePrefixes::TempSahyadriConsensusCompact.into_iter().chain(lvl_bytes).chain(temp_index_bytes).collect_vec();
        Self {
            db: Arc::clone(&db),
            level,
            access: CachedDbAccess::new(db.clone(), cache_policy, prefix),
    ...
    pub fn insert_batch(&self, batch: &mut WriteBatch, hash: Hash, data: &Arc<SahyadriConsensusData>) -> Result<(), StoreError> {
    ...
    pub fn update_batch(&self, batch: &mut WriteBatch, hash: Hash, data: &Arc<SahyadriConsensusData>) -> Result<(), StoreError> {
        self.access.write(BatchDbWriter::new(batch), hash, data.clone())?;
        self.compact_access.write(BatchDbWriter::new(batch), hash, data.to_compact())?;
        Ok(())
    }
...
impl SahyadriConsensusStoreReader for DbSahyadriConsensusStore {
    fn get_blue_score(&self, hash: Hash) -> Result<u64, StoreError> {
        if let Some(sahyadri_consensus_data) = self.access.read_from_cache(hash) {
            return Ok(sahyadri_consensus_data.blue_score);
        }
    ...
    fn get_blue_work(&self, hash: Hash) -> Result<BlueWorkType, StoreError> {
        if let Some(sahyadri_consensus_data) = self.access.read_from_cache(hash) {
            return Ok(sahyadri_consensus_data.blue_work);
        }
    ...
    fn get_selected_parent(&self, hash: Hash) -> Result<Hash, StoreError> {
        if let Some(sahyadri_consensus_data) = self.access.read_from_cache(hash) {
            return Ok(sahyadri_consensus_data.selected_parent);
        }
...
impl SahyadriConsensusStore for DbSahyadriConsensusStore {
    fn insert(&self, hash: Hash, data: Arc<SahyadriConsensusData>) -> Result<(), StoreError> {
        if self.access.has(hash)? {
            return Err(StoreError::HashAlreadyExists(hash));
        }
...
/// An in-memory implementation of `SahyadriConsensusStore` trait to be used for tests.
/// Uses `RefCell` for interior mutability in order to workaround `insert`
/// being non-mutable.
pub struct MemorySahyadriConsensusStore {
...
impl MemorySahyadriConsensusStore {
    pub fn new() -> Self {
        Self {
    ...
    pub fn key_not_found_error(hash: Hash) -> StoreError {
        StoreError::KeyNotFound(DbKey::new(DatabaseStorePrefixes::SahyadriConsensus.as_ref(), hash))
    }
}

impl Default for MemorySahyadriConsensusStore {
    fn default() -> Self {
        Self::new()
    }
}

impl SahyadriConsensusStore for MemorySahyadriConsensusStore {
    fn insert(&self, hash: Hash, data: Arc<SahyadriConsensusData>) -> Result<(), StoreError> {
        if self.has(hash)? {
            return Err(StoreError::HashAlreadyExists(hash));
        }
...
impl SahyadriConsensusStoreReader for MemorySahyadriConsensusStore {
    fn get_blue_score(&self, hash: Hash) -> Result<u64, StoreError> {
    ...
    fn get_data(&self, hash: Hash) -> Result<Arc<SahyadriConsensusData>, StoreError> {
        ...
        Ok(Arc::new(SahyadriConsensusData::new(
...
mod tests {
    ...
    fn test_mergeset_iterators() {
        let store = MemorySahyadriConsensusStore::new();

        let factory = |w: u64| {
            Arc::new(SahyadriConsensusData {
                blue_score: Default::default(),
                blue_work: w.into(),
        ...
        let mut data = SahyadriConsensusData::new_with_selected_parent(1.into(), 5);
