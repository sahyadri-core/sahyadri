...
use sahyadri_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use sahyadri_database::prelude::{DB, StoreResult};
use sahyadri_database::prelude::{DirectWriter, MemoryWriter};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; sahyadri_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use sahyadri_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(sahyadri_database::prelude::ConnBuilder::default().with_files_limit(10));
