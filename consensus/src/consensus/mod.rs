...
impl Consensus {
    ...
    pub fn intrusive_pruning_point_store_writes(
        ...
        let virtual_state = Arc::new(VirtualState {
            ...
            sahyadri_consensus_data: self.services.sahyadri_consensus_manager.sahyadri_consensus(&virtual_parents),
    ...
    fn get_and_verify_path_to_new_pruning_point(&self, new_pruning_point: Hash, syncer_sink: Hash) -> ConsensusResult<VecDeque<Hash>> {
        ...
        let Ok(candidate_sahyadri_consensus_data) = self.get_sahyadri_consensus_data(new_pruning_point) else {
        ...
        self.services
            ...
            .is_pruning_sample(
                candidate_sahyadri_consensus_data.blue_score,
                selected_parent_sahyadri_consensus_data.blue_score,
...
impl ConsensusApi for Consensus {
    ...
    fn get_virtual_merge_depth_root(&self) -> Option<Hash> {
        ...
        let virtual_sahyadri_consensus_data = &virtual_state.sahyadri_consensus_data;
        let root = self.services.depth_manager.calc_merge_depth_root(virtual_sahyadri_consensus_data, pruning_point);
    ...
    fn get_current_block_color(&self, hash: Hash) -> Option<bool> {
        ...
        // Verify that the block is in future(retention root), where SahyadriConsensus data is complete
        ...
        for child in initial_children {
            if visited.insert(child) {
                let blue_work = self.sahyadri_consensus_store.get_blue_work(child).unwrap();
        ...
        while let Some(Reverse(SortableBlock { hash: decedent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(decedent, sink) {
                let decedent_data = self.get_sahyadri_consensus_data(decedent).unwrap();
                ...
                // unreachable.
                sahyadri_core::warn!("DAG topology inconsistency: {decedent} is expected to be a merging block of {hash}");
    ...
    fn get_chain_block_samples(&self) -> Vec<DaaScoreTimestamp> {
        ...
        if self.config.net.network_type == NetworkType::Mainnet {
            ...
            // Source: https://github.com/sahyadrigang/sahyadrid-py-explorer/blob/main/src/tx_timestamp_estimation.ipynb
            // For context see also: https://github.com/sahyadrigang/sahyadrid-py-explorer/blob/main/src/genesis_proof.ipynb
    ...
    fn get_sahyadri_consensus_data(&self, hash: Hash) -> ConsensusResult<ExternalSahyadriConsensusData> {
    ...
    fn estimate_network_hashes_per_second(&self, start_hash: Option<Hash>, window_size: usize) -> ConsensusResult<u64> {
        ...
        match start_hash {
            Some(hash) => {
                ...
                if !self.headers_store.has(sahyadri_consensus_data.selected_parent).unwrap() {
                    return Err(ConsensusError::DifficultyError(DifficultyError::InsufficientWindowData(0)));
                }
                self.estimate_network_hashes_per_second_impl(&sahyadri_consensus_data, window_size)
            }
            None => {
                let virtual_state = self.lkg_virtual_state.load();
                self.estimate_network_hashes_per_second_impl(&virtual_state.sahyadri_consensus_data, window_size)
            }
        }
    }
