...
use sahyadri_consensus_notify::{notification::Notification, root::ConsensusNotificationRoot};
use sahyadri_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl};
use sahyadri_core::{core::Core, service::Service};
...
impl TestConsensus {
    ...
    pub fn build_header_with_parents(&self, hash: Hash, parents: Vec<Hash>) -> Header {
        ...
        let sahyadri_consensus_data = self.consensus.services.sahyadri_consensus_manager.sahyadri_consensus(header.direct_parents());
        let daa_window = self.consensus.services.window_manager.block_daa_window(&sahyadri_consensus_data).unwrap();
        header.bits = self.consensus.services.window_manager.calculate_difficulty_bits(&sahyadri_consensus_data, &daa_window);
        header.daa_score = daa_window.daa_score;
        header.timestamp = self.consensus.services.window_manager.calc_past_median_time(&sahyadri_consensus_data).unwrap().0 + 1;
        header.blue_score = sahyadri_consensus_data.blue_score;
        header.blue_work = sahyadri_consensus_data.blue_work;
    ...
    pub fn sahyadri_consensus_store(&self) -> &Arc<DbSahyadriConsensusStore> {
        &self.consensus.sahyadri_consensus_store
    }
    ...
    pub fn sahyadri_consensus_manager(&self) -> &DbSahyadriConsensusManager {
        &self.consensus.services.sahyadri_consensus_manager
    }
}
