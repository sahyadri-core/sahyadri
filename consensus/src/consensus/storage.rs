...
pub struct ConsensusStorage {
    ...
    // Append-only stores
    pub sahyadri_consensus_store: Arc<DbSahyadriConsensusStore>,
...
impl ConsensusStorage {
    pub fn new(db: Arc<DB>, config: Arc<Config>) -> Arc<Self> {
        ...
        let sahyadri_consensus_store = Arc::new(DbSahyadriConsensusStore::new(
            db.clone(),
            0,
            sahyadri_consensus_builder.downscale(0).build(),
            sahyadri_consensus_compact_builder.downscale(0).build(),
        ));
