...
use sahyadri_consensus_core::{api::ConsensusApi, config::Config, mining_rules::MiningRules};
use sahyadri_consensus_notify::root::ConsensusNotificationRoot;
use sahyadri_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl, SessionLock};
use sahyadri_core::{debug, time::unix_now, warn};
...
impl ConsensusFactory for Factory {
    fn new_active_consensus(&self) -> (ConsensusInstance, DynConsensusCtl) {
        ...
        let db = sahyadri_database::prelude::ConnBuilder::default()
            .with_db_path(dir)
