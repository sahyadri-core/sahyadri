...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'sahyadri'.")]
...
///
/// NetworkId is a unique identifier for a sahyadri network instance.
...
impl NetworkId {
    ...
    /// Returns a textual description of the network prefixed with `sahyadri-`
    pub fn to_prefixed(&self) -> String {
        format!("sahyadri-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("sahyadri-") {
