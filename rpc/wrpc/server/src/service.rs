...
/// ### SahyadriRpcHandler
///
/// [`SahyadriRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct SahyadriRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl SahyadriRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        SahyadriRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for SahyadriRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "csm" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<SahyadriRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(SahyadriRpcHandler::new(tasks, *encoding, core_service, options.clone()));
