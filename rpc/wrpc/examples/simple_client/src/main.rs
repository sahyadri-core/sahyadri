// Example of simple client to connect with Sahyadri node using wRPC connection and collect some node and network basic data
...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to Sahyadri node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // Verify your Sahyadri node is runnning with --rpclisten-borsh=0.0.0.0:17110 parameter
    ...
    // Define the network your Sahyadri node is connected to
    ...
    let client = SahyadriRpcClient::new(encoding, url, resolver, selected_network, subscription_context)?;
    ...
    // Connect to selected Sahyadri node
    ...
    let GetSahyadriDagInfoResponse {
        ...
        sink,
        ..
    } = client.get_sahyadri_dag_info().await?;
    ...
    // Disconnect client from Sahyadri node
