...
use sahyadri_rpc_core::{RpcDataVerbosityLevel, RpcHash, RpcOptionalTransaction, api::rpc::RpcApi};
use sahyadri_wrpc_client::{
    SahyadriRpcClient, WrpcEncoding,
...
async fn get_vcc_v2() -> Result<()> {
    ...
    let dag_info = client.get_sahyadri_dag_info().await?;
