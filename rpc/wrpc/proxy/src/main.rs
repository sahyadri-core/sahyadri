...
struct Args {
    ...
    /// proxy:port for gRPC server (grpc://127.0.0.1:26110)
    #[clap(name = "grpc")]
...
async fn main() -> Result<()> {
    ...
    let sahyadrid_port = network_type.default_rpc_port();
    ...
    let options = Arc::new(Options {
        ...
        grpc_proxy_address: Some(grpc_proxy_address.unwrap_or_else(|| format!("grpc://127.0.0.1:{sahyadrid_port}"))),
        verbose,
        // ..Options::default()
    });
    log_info!("");
    ...
    let rpc_handler = Arc::new(SahyadriRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Sahyadri wRPC server is listening on {}", options.listen_address);
