//!
//! Module implementing [`Resolver`] client for obtaining public Sahyadri wRPC endpoints.
//!
...
///
/// # Resolver - a client for obtaining public Sahyadri wRPC endpoints.
///
/// This client operates against [Sahyadri Resolver](https://github.com/aspectron/sahyadri-resolver) service
/// that provides load-balancing and failover capabilities for Sahyadri wRPC endpoints. The default
/// configuration allows access to public Sahyadri nodes, while custom configurations can be supplied
/// if you are running your own custom Sahyadri node cluster.
///
#[derive(Debug, Clone)]
pub struct Resolver {
    inner: Arc<Inner>,
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/sahyadri/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Sahyadri p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Sahyadri wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
