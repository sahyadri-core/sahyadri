...
use sahyadri_consensus_core::block::{Block, MutableBlock};
...
impl From<&Block> for RpcBlock {
    fn from(item: &Block) -> Self {
        Self {
            ...
            // TODO: Implement a populating process inspired from sahyadrid\app\rpc\rpccontext\verbosedata.go
            verbose_data: None,
        }
    }
}
    ...
    fn try_from(item: RpcBlock) -> RpcResult<Self> {
        Ok(Self {
            ...
            transactions: Arc::new(
                item.transactions
                    .into_iter()
                    .map(sahyadri_consensus_core::tx::Transaction::try_from)
                    .collect::<RpcResult<Vec<sahyadri_consensus_core::tx::Transaction>>>()?,
            ),
        })
    }
}
