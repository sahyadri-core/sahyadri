...
pub struct GetBlockTemplateRequest {
    /// Which sahyadri address should the coinbase block reward transaction pay into
...
pub struct GetBlockTemplateResponse {
    ...
    /// Whether sahyadrid thinks that it's synced.
    /// Callers are discouraged (but not forbidden) from solving blocks when sahyadrid is not synced.
    /// That is because when sahyadrid isn't in sync with the rest of the network there's a high
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetSahyadriDagInfoRequest {}

impl Serializer for GetSahyadriDagInfoRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        Ok(())
    }
...
impl Deserializer for GetSahyadriDagInfoRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        Ok(Self {})
    }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetSahyadriDagInfoResponse {
...
impl GetSahyadriDagInfoResponse {
    pub fn new(
...
impl Serializer for GetSahyadriDagInfoResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
...
impl Deserializer for GetSahyadriDagInfoResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
...
pub struct GetCoinSupplyResponse {
    pub max_kana: u64,
    pub circulating_kana: u64,
}

impl GetCoinSupplyResponse {
    pub fn new(max_kana: u64, circulating_kana: u64) -> Self {
        Self { max_kana, circulating_kana }
    }
}
...
impl Serializer for GetCoinSupplyResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        store!(u64, &self.max_kana, writer)?;
        store!(u64, &self.circulating_kana, writer)?;
...
impl Deserializer for GetCoinSupplyResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let max_kana = load!(u64, reader)?;
        let circulating_kana = load!(u64, reader)?;

        Ok(Self { max_kana, circulating_kana })
    }
}
...
#[derive(Clone, Debug, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
pub struct SubmitAccountTransactionResponse {
...
impl Serializer for SubmitAccountTransactionRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?; // Version 1
        store!(String, &self.sender, writer)?;
        store!(String, &self.receiver, writer)?;
...
impl Deserializer for SubmitAccountTransactionRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
...
impl Serializer for SubmitAccountTransactionResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?; // Version 1
...
impl Deserializer for SubmitAccountTransactionResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
