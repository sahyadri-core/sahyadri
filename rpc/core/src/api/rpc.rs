...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known sahyadrid addresses in the current network (mainnet, testnet, etc.)
    ...
    async fn get_sahyadri_dag_info(&self) -> RpcResult<GetSahyadriDagInfoResponse> {
        self.get_sahyadri_dag_info_call(None, GetSahyadriDagInfoRequest {}).await
    }
    async fn get_sahyadri_dag_info_call(
        &self,
        connection: Option<&DynRpcConnection>,
        request: GetSahyadriDagInfoRequest,
