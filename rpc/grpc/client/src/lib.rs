...
use sahyadri_utils::{channel::Channel, triggers::DuplexTrigger};
use sahyadri_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(SahyadridPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
    ...
    route!(get_sahyadri_dag_info_call, GetSahyadriDagInfo);
...
type SahyadridRequestSender = async_channel::Sender<SahyadridRequest>;
type SahyadridRequestReceiver = async_channel::Receiver<SahyadridRequest>;
...
/// Data flow:
/// ```
/// //   SahyadridRequest -> request_send -> stream -> SahyadridResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: SahyadridRequestSender,
    request_receiver: SahyadridRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<SahyadridResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual SahyadridRequest to SahyadridResponse stream
        let mut stream: Streaming<SahyadridResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: SahyadridPayloadOps, request: impl Into<SahyadridRequest>) -> Result<SahyadridResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: SahyadridRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<SahyadridResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = sahyadrid_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = sahyadrid_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
