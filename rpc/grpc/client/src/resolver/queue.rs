...
struct Pending {
    ...
    sender: SahyadridResponseSender,
}

impl Pending {
    fn new(op: SahyadridPayloadOps, request: SahyadridRequest, sender: SahyadridResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &SahyadridResponse, response_op: SahyadridPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: SahyadridPayloadOps, request: &SahyadridRequest) -> SahyadridResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<SahyadridResponse>>();
        {
    ...
    fn handle_response(&self, response: SahyadridResponse) {
        let response_op: SahyadridPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");
