...
fn compressed_parents_to_protowire(parents: &sahyadri_rpc_core::RpcCompressedParents) -> Vec<protowire::RpcBlockLevelRun> {
    parents
        .raw()
        .iter()
...
fn compressed_parents_from_protowire(runs: &[protowire::RpcBlockLevelRun]) -> RpcResult<sahyadri_rpc_core::RpcCompressedParents> {
...
from!(item: &sahyadri_rpc_core::RpcOptionalHeader, protowire::RpcOptionalHeader, {
    Self {
...
try_from!(item: &protowire::RpcOptionalHeader, sahyadri_rpc_core::RpcOptionalHeader, {
    Self {
        ...
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: item.blue_work.as_ref().map(|x| sahyadri_rpc_core::RpcBlueWorkType::from_rpc_hex(x)).transpose()?,
        blue_score: item.blue_score,
