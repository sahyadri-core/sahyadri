...
use sahyadri_grpc_core::{
    ...
    protowire::{
        SahyadridRequest, SahyadridResponse,
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<SahyadridResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<SahyadridRequest>>,
