...
struct RpcTable {
    ...
    sahyadrid_request_type: Expr,
    sahyadrid_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, SahyadridRequestType, SahyadridResponseType, SahyadridPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let sahyadrid_request_type = iter.next().unwrap().clone();
        let sahyadrid_response_type = iter.next().unwrap().clone();
        ...
        Ok(RpcTable {
            ...
            server_ctx_type,
            connection_ctx_type,
            sahyadrid_request_type,
            sahyadrid_response_type,
            payload_ops,
            handlers,
        })
    }
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let sahyadrid_request_type = &self.sahyadrid_request_type;
        let sahyadrid_response_type = &self.sahyadrid_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #sahyadrid_request_type, #sahyadrid_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #sahyadrid_request_type| {
                                Box::pin(async move {
                                    let mut response: #sahyadrid_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #sahyadrid_request_type, #sahyadrid_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #sahyadrid_request_type| {
                                Box::pin(async move {
                                    let mut response: #sahyadrid_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match sahyadri_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
