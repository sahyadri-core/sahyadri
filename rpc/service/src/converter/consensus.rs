...
use sahyadri_addresses::{Address, AddressError};
...
use sahyadri_consensusmanager::{ConsensusManager, ConsensusProxy};
use sahyadri_hashes::Hash;
use sahyadri_math::Uint256;
use sahyadri_mining::model::{TransactionIdSet, owner_txs::OwnerTransactions};
...
impl ConsensusConverter {
    ...
    ///
    /// _GO-SAHYADRID: PopulateBlockWithVerboseData_
    pub async fn get_block(
        ...
        let hash = block.hash();
        let sahyadri_consensus_data = consensus.async_get_sahyadri_consensus_data(hash).await?;
        ...
        let verbose_data = Some(RpcBlockVerboseData {
            ...
            selected_parent_hash: sahyadri_consensus_data.selected_parent,
            transaction_ids: block.transactions.iter().map(|x| x.id()).collect(),
            is_header_only: block_status.is_header_only(),
            blue_score: sahyadri_consensus_data.blue_score,
            children_hashes: children,
            merge_set_blues_hashes: sahyadri_consensus_data.mergeset_blues,
            merge_set_reds_hashes: sahyadri_consensus_data.mergeset_reds,
            is_chain_block,
        });
    ...
    ///
    /// _GO-SAHYADRID: PopulateTransactionWithVerboseData
