...
use sahyadri_p2p_flows::flow_context::FlowContext;
use sahyadri_p2p_lib::common::ProtocolError;
use sahyadri_p2p_mining::rule_engine::MiningRuleEngine;
use sahyadri_perf_monitor::{Monitor as PerfMonitor, counters::CountersSnapshot};
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. This is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(sahyadri_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }
        ...
        let script_public_key = sahyadri_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn get_blocks_call(
        ...
        let low_hash = match request.low_hash {
            Some(low_hash) => {
                ...
                session.async_get_sahyadri_consensus_data(low_hash).await?;
                low_hash
            }
    ...
    async fn get_sink_blue_score_call(
        ...
        let session = self.consensus_manager.consensus().unguarded_session();
        Ok(GetSinkBlueScoreResponse::new(session.async_get_sahyadri_consensus_data(session.async_get_sink().await).await?.blue_score))
    }
    ...
    async fn get_utxos_by_addresses_call(
        ...
        for address in request.addresses {
            let balance = session.get_account_balance(&address).unwrap_or(0);
    ...
    async fn get_balance_by_address_call(
        ...
        let session = self.consensus_manager.consensus().unguarded_session();
        let real_balance = session.get_account_balance(&address).unwrap_or(0);
    ...
    async fn get_sahyadri_dag_info_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        _: GetSahyadriDagInfoRequest,
    ) -> RpcResult<GetSahyadriDagInfoResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        ...
        Ok(GetSahyadriDagInfoResponse::new(
            self.config.net,
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: sahyadri_hashes::Hash = sahyadri_hashes::Hash::from_bytes([0xff; sahyadri_hashes::HASH_SIZE]);
