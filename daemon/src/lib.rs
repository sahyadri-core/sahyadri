...
pub use sahyadrid::{Sahyadrid, SahyadridConfig, SahyadridCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../sahyadri-cpu-miner/target/debug",
    "../../sahyadri-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub sahyadrid: Option<Arc<dyn SahyadridCtl + Send + Sync + 'static>>,
    // pub sahyadrid_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { sahyadrid: None, cpu_miner: None }
    }

    pub fn with_sahyadrid(mut self, sahyadrid: Arc<dyn SahyadridCtl + Send + Sync + 'static>) -> Self {
        self.sahyadrid = Some(sahyadrid);
        self
    }
    ...
    pub fn sahyadrid(&self) -> Arc<dyn SahyadridCtl + Send + Sync + 'static> {
        self.sahyadrid.as_ref().expect("accessing Daemons::sahyadrid while sahyadrid option is None").clone()
    }

    pub fn try_sahyadrid(&self) -> Option<Arc<dyn SahyadridCtl + Send + Sync + 'static>> {
        self.sahyadrid.clone()
    }
