...
impl Default for SahyadridConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<SahyadridConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: SahyadridConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no sahyadrid path is specified".to_string()));
        }
...
pub struct Sahyadrid {
    inner: Arc<Mutex<Inner>>,
    mute: Arc<AtomicBool>,
    events: Channel<ProcessEvent>,
}

impl Default for Sahyadrid {
    fn default() -> Self {
...
impl Sahyadrid {
    pub fn new(args: SahyadridConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: SahyadridConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process
            && process.is_running()
        {
            return Err(Error::Custom("Sahyadri node is already running.".to_string()));
        }
        ...
        // let options = SahyadridOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Sahyadrid binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait SahyadridCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: SahyadridConfig) -> Result<()>;
