...
impl From<&SahyadriCli> for Arc<Terminal> {
    fn from(ctx: &SahyadriCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<SahyadriCli> for SahyadriCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for SahyadriCli {
...
impl SahyadriCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                sahyadri_core::log::init_logger(None, "info");
            } else {
                sahyadri_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let sahyadri_cli = Arc::new(SahyadriCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(sahyadri_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                sahyadri_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<SahyadriRpcClient>> {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletList { .. } => {},
                                Events::WalletPing => {
                                    // log_info!("Sahyadri NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Sahyadri NG - received metrics event {metrics:?}")
                                }
                                Events::FeeRate { .. } => {},
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Sahyadri node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let _is_open = this.wallet.is_open();

                                  //  if !is_synced {
                                  //     if is_open {
                                  //          terrorln!(this, "Unable to update the wallet state - Sahyadri node is currently syncing with the network...");

                                  //      } else {
                                  //          terrorln!(this, "Sahyadri node is currently syncing with the network, please wait for the sync to complete...");
                                  //      }
                                  //  }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let sahyadrid = self.daemons().try_sahyadrid();
            ...
            if let Some(sahyadrid) = sahyadrid.as_ref() {
                sahyadrid.mute(false).await?;
...
impl Cli for SahyadriCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "csm" {
                prompt.push(title);
            }
...
impl cli::Context for SahyadriCli {
...
pub async fn sahyadri_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    SahyadriCli::init();

    let options = Options::new(terminal_options, None);
    let cli = SahyadriCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Sahyadri Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
    ...
    // --- Sahyadri Global Auto-Connect End ---

    // terminal blocks async execution (Ab terminal chalu hoga)
    cli.run().await?;
