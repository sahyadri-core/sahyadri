...
use sahyadri_wallet_core::error::Error as WalletError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WrpcError(#[from] sahyadri_wrpc_client::error::Error),

    #[error(transparent)]
    RpcError(#[from] sahyadri_rpc_core::RpcError),
    ...
    #[error(transparent)]
    AddressError(#[from] sahyadri_addresses::AddressError),
    ...
    #[error(transparent)]
    Daemon(#[from] sahyadri_daemon::error::Error),
    ...
    #[error(transparent)]
    Bip32(#[from] sahyadri_bip32::Error),

    #[error("private key {0} already exists")]
    PrivateKeyAlreadyExists(String),

    #[error(transparent)]
    MetricsError(sahyadri_metrics_core::error::Error),

    #[error(transparent)]
    SahyadriWalletKeys(#[from] sahyadri_wallet_keys::error::Error),

    #[error(transparent)]
    PskbLockScriptSigError(#[from] sahyadri_wallet_pskt::error::Error),
