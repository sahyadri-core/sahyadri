use sahyadri_wallet_core::account::BIP32_ACCOUNT_KIND;
use sahyadri_wallet_core::account::LEGACY_ACCOUNT_KIND;
use sahyadri_wallet_core::account::MULTISIG_ACCOUNT_KIND;
                    ...
                    ctx.term().help(
                        &[
                            ...
                            (
                                ...
                                "Import accounts 12 word mnemonic used by legacy applications (KDX and sahyadrinet web wallet)",
                            ),
                            (
                ...
                match import_kind.as_ref() {
                    "legacy-data" => {
                        ...
                        if exists_legacy_v0_keydata().await? {
                                ...
                                .import_legacy_keydata(
                                    ...
                                    Some(Arc::new(move |processed: usize, _, balance, txid| {
                                        if let Some(txid) = txid {
                                            tprintln!(
                                                ctx_,
                                                "Scan detected {} CSM at index {}; transfer txid: {}",
                            ...
                            return Err("'sahyadrinet' web wallet storage not found at this domain name".into());
                        } else {
                    ...
                    "mnemonic" => {
                        if argv.is_empty() {
                            ...
                            tprintln!(ctx, "please use 'legacy' for 12-word KDX and sahyadrinet web wallet mnemonics\r\n");
                            return Ok(());
                        }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<SahyadriCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (KDX and sahyadrinet web wallet). Use 'account import' for additional help.",
                ),
            ...
            .derivation_scan(
                ...
                Some(Arc::new(move |processed: usize, _, balance, txid| {
                    if let Some(txid) = txid {
                        ...
                        tprintln!(ctx_, "Scanned {} derivations, found {} CSM", processed, kana_to_sahyadri_string(balance));
                    }
                })),
            )
            .await?;
