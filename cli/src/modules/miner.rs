use crate::imports::*;
use sahyadri_daemon::{CpuMinerConfig, locate_binaries};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "sahyadri-cpu-miner").await
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<SahyadriCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<SahyadriCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<SahyadriCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = sahyadri_daemon::locate_binaries(root.as_str(), "sahyadri-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No sahyadri-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select sahyadri-cpu-miner binary", &binaries).await? {
