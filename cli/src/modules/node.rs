use crate::imports::*;
use sahyadri_daemon::SahyadridConfig;
...
#[async_trait]
impl DefaultSettings for SahyadridSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        let mut settings = vec![(Self::Mute, to_value(true).unwrap())];

        let root = nw_sys::app::folder();
        if let Ok(binaries) = sahyadri_daemon::locate_binaries(&root, "sahyadrid").await
...
pub struct Node {
    settings: SettingsStore<SahyadridSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("sahyadrid").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<SahyadriCli>() { ctx.daemons().clone().sahyadrid.as_ref().map(|_| "node") } else { None }
    }

    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local Sahyadri node instance"
    }

    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        self.settings.try_load().await.ok();
        if let Some(mute) = self.settings.get(SahyadridSettings::Mute) {
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<SahyadriCli>) -> Result<SahyadridConfig> {
        let location: String = self
            .settings
            .get(SahyadridSettings::Location)
        ...
        let mute = false;
        let config = SahyadridConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<SahyadriCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let sahyadrid = ctx.daemons().sahyadrid();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting sahyadri node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting sahyadri node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                sahyadrid.configure(self.create_config(&ctx).await?).await?;
                sahyadrid.start().await?;
            ...
            "stop" => {
                sahyadrid.stop().await?;
            }
            "restart" => {
                sahyadrid.configure(self.create_config(&ctx).await?).await?;
                sahyadrid.restart().await?;
            }
            "kill" => {
                sahyadrid.kill().await?;
            }
            "mute" | "logs" => {
                let mute = !self.mute.load(Ordering::SeqCst);
                ...
                // sahyadrid.mute(mute).await?;
                self.settings.set(SahyadridSettings::Mute, mute).await?;
            }
            "status" => {
                let status = sahyadrid.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                sahyadrid.configure(self.create_config(&ctx).await?).await?;
                let version = sahyadrid.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<SahyadriCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Sahyadrid executable (binary) location"),
                ("version", "Display Sahyadrid executable version"),
                ("start", "Start the local Sahyadri node instance"),
                ("stop", "Stop the local Sahyadri node instance"),
                ("restart", "Restart the local Sahyadri node instance"),
                ("kill", "Kill the local Sahyadri node instance"),
                ("status", "Get the status of the local Sahyadri node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<SahyadriCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = sahyadri_daemon::locate_binaries(root.as_str(), "sahyadrid").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No sahyadrid binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a sahyadrid binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(SahyadridSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(SahyadridSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/sahyadrid'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<SahyadriCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Sahyadrid has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Sahyadrid error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
