...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<SahyadriCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<SahyadriCli>, sahyadri_address: &str, message: &str) -> Result<()> {
        let sahyadri_address = Address::try_from(sahyadri_address)?;
        if sahyadri_address.version != Version::PubKey {
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<SahyadriCli>, sahyadri_address: &str, signature: &str, message: &str) -> Result<()> {
        let sahyadri_address = Address::try_from(sahyadri_address)?;
        if sahyadri_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<SahyadriCli>, sahyadri_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&sahyadri_address])?;
