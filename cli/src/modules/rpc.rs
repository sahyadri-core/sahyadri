...
#[help("Execute RPC commands against the connected Sahyadri node")]
pub struct Rpc;

impl Rpc {
    fn println<T>(&self, ctx: &Arc<SahyadriCli>, v: T)
    where
        T: core::fmt::Debug,
    {
    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        ...
        match op {
            ...
            RpcApiOps::GetSahyadriDagInfo => {
                let result = rpc.get_sahyadri_dag_info_call(None, GetSahyadriDagInfoRequest {}).await?;
                self.println(&ctx, result);
            }
            ...
            RpcApiOps::GetBalanceByAddress => {
                ...
                for address in addresses {
                    ...
                    self.println(&ctx, kana_to_sahyadri(result.balance));
                }
            }
