...
use sahyadri_consensus::{
    ...
    model::stores::{
        ...
        sahyadri_consensus::{KType, SahyadriConsensusStoreReader},
    },
    params::{DEVNET_PARAMS, ForkActivation, NETWORK_DELAY_BOUND, OverrideParams, Params, SIMNET_PARAMS},
};
use sahyadri_consensus_core::{
    ...
    config::bps::calculate_sahyadri_consensus_k, errors::block::BlockProcessResult, mining_rules::MiningRules, tx::TransactionType,
};
...
use sahyadri_database::{create_temp_db, load_existing_db};
use sahyadri_hashes::Hash;
use sahyadri_perf_monitor::{builder::Builder, counters::CountersSnapshot};
use sahyadri_utils::fd_budget;
use simulator::network::SahyadriNetworkSimulator;
...
fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("yugat-heap.json").build();
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "semaphore-trace")] {
            sahyadri_core::log::init_logger(None, &format!("{},{}=debug", args.log_level, sahyadri_utils::sync::semaphore_module_path()));
        } else {
            sahyadri_core::log::init_logger(None, &args.log_level);
        }
    };
...
fn main_impl(mut args: Args) {
    ...
    let stop_perf_monitor = args.perf_metrics.then(|| {
        ...
        let cb = move |counters: CountersSnapshot| {
            trace!("[{}] {}", sahyadri_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            trace!("[{}] {}", sahyadri_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
    ...
    args.bps = if args.testnet11 { 1.0 } else { args.bps };
    let mut params = if args.testnet11 { SIMNET_PARAMS } else { DEVNET_PARAMS };
    params.raigad_activation = ForkActivation::always();
    ...
    let (consensus, _lifetime) = if let Some(input_dir) = args.input_dir {
        ...
        let mut sim = SahyadriNetworkSimulator::new(args.delay, args.bps, args.target_blocks, config.clone(), args.output_dir);
    ...
    if args.test_pruning {
        ...
        let num_txs = print_stats(&consensus, &hashes, args.delay, args.bps, config.sahyadri_consensus_k());
...
fn apply_args_to_consensus_params(args: &Args, params: &mut Params) {
    ...
    if args.testnet11 {
        info!(
            "Using sahyadri-testnet-11 configuration (SAHYADRI_CONSENSUS K={}, DAA window size={}, Median time window size={})",
        ...
        let k = u64::max(calculate_sahyadri_consensus_k(2.0 * max_delay * args.bps, 0.05), params.sahyadri_consensus_k() as u64);
        let k = u64::min(k, KType::MAX as u64) as KType; // Clamp to KType::MAX
        params.sahyadri_consensus_k = k;
        ...
        if args.daa_legacy {
            ...
            params.raigad_activation = ForkActivation::never();
        ...
        info!("2Dλ={}, SAHYADRI_CONSENSUS K={}, DAA window size={}", 2.0 * args.delay * args.bps, k, params.difficulty_window_size);
    }
    if args.test_pruning {
        ...
        params.sahyadri_consensus_k = 20;
...
async fn validate(src_consensus: &Consensus, dst_consensus: &Consensus, params: &Params, delay: f64, bps: f64, header_only: bool) {
    ...
    let num_blocks = hashes.len();
    let num_txs = print_stats(src_consensus, &hashes, delay, bps, params.sahyadri_consensus_k());
    if header_only {
...
fn print_stats(src_consensus: &Consensus, hashes: &[Hash], delay: f64, bps: f64, k: KType) -> usize {
    let blues_mean =
        hashes.iter().map(|&h| src_consensus.sahyadri_consensus_store.get_data(h).unwrap().mergeset_blues.len()).sum::<usize>() as f64
            / hashes.len() as f64;
    let reds_mean =
        hashes.iter().map(|&h| src_consensus.sahyadri_consensus_store.get_data(h).unwrap().mergeset_reds.len()).sum::<usize>() as f64
            / hashes.len() as f64;
    ...
    info!("[DELAY={delay}, BPS={bps}, SAHYADRI_CONSENSUS K={k}]");
...
mod tests {
    ...
    fn test_pruning_via_yugat() {
        ...
        sahyadri_core::log::try_init_logger(&args.log_level);
        // As we log the panic, we want to set it up after the logger
        sahyadri_core::panic::configure_panic();
        main_impl(args);
    }
