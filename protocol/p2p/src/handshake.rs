...
/// Implements the Sahyadri peer-to-peer handshake protocol
pub struct SahyadridHandshake<'a> {
    router: &'a Router,
...
impl<'a> SahyadridHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![SahyadridMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![SahyadridMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![SahyadridMessagePayloadType::Ready]),
        }
    }
