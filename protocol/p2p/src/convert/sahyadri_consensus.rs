...
impl From<&ExternalSahyadriConsensusData> for protowire::SahyadriConsensusData {
    fn from(item: &ExternalSahyadriConsensusData) -> Self {
        Self {
...
impl TryFrom<protowire::SahyadriConsensusData> for ExternalSahyadriConsensusData {
    type Error = ConversionError;
    fn try_from(item: protowire::SahyadriConsensusData) -> Result<Self, Self::Error> {
...
impl TryFrom<protowire::BlockSahyadriConsensusDataHashPair> for TrustedSahyadriConsensusData {
    type Error = ConversionError;
    fn try_from(pair: protowire::BlockSahyadriConsensusDataHashPair) -> Result<Self, Self::Error> {
        Ok(Self::new(pair.hash.try_into_ex()?, pair.sahyadri_consensus_data.try_into_ex()?))
    }
}

impl TryFrom<Versioned<protowire::DaaBlockV4>> for TrustedHeader {
    type Error = ConversionError;
    fn try_from(value: Versioned<protowire::DaaBlockV4>) -> Result<Self, Self::Error> {
        ...
        Ok(TrustedHeader::new(Versioned(header_format, header).try_into().map(Arc::new)?, b.sahyadri_consensus_data.try_into_ex()?))
    }
}
