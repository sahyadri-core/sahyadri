use crate::{SahyadridMessagePayloadType, convert::error::ConversionError, core::peer::PeerKey};
use sahyadri_consensus_core::errors::{block::RuleError, consensus::ConsensusError, pruning::PruningImportError};
use sahyadri_mining_errors::manager::MiningManagerError;
...
pub enum ProtocolError {
    ...
    UnexpectedMessage(&'static str, Option<SahyadridMessagePayloadType>),
    ...
    IncomingRouteCapacityReached(SahyadridMessagePayloadType, String),
    ...
    NoRouteForMessageType(SahyadridMessagePayloadType),
...
/// Wraps an inner payload message into a valid `SahyadridMessage`.
/// Usage:
/// ```ignore
...
macro_rules! make_message {
    ...
    ($pattern:path, $msg:expr, $response_id:expr, $request_id: expr) => {{ $crate::pb::SahyadridMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: $request_id } }};
}

#[macro_export]
macro_rules! make_response {
    ($pattern:path, $msg:expr, $response_id:expr) => {{ $crate::pb::SahyadridMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: 0 } }};
}

#[macro_export]
macro_rules! make_request {
    ($pattern:path, $msg:expr, $request_id:expr) => {{ $crate::pb::SahyadridMessage { payload: Some($pattern($msg)), response_id: 0, request_id: $request_id } }};
}

/// Macro to extract a specific payload type from an `Option<pb::SahyadridMessage>`.
/// Usage:
/// ```ignore
...
/// Macro to await a channel `Receiver<pb::SahyadridMessage>::recv` call with a default/specified timeout and expect a specific payload type.
/// Usage:
/// ```ignore
...
/// Macro to indefinitely await a channel `Receiver<pb::SahyadridMessage>::recv` call and expect a specific payload type (without a timeout).
/// Usage:
/// ```ignore
