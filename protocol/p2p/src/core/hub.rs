...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: SahyadridMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: SahyadridMessage, filter_peer: Option<PeerKey>) {
        let peers = self
            .peers
            .read()
            .values()
    ...
    pub async fn broadcast_to_some_peers(&self, msg: SahyadridMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<SahyadridMessage>, filter_peer: Option<PeerKey>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self
