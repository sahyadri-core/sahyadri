...
use sahyadri_consensusmanager::{ConsensusProxy, StagingConsensus, spawn_blocking};
use sahyadri_core::{debug, info, time::unix_now, warn};
...
use sahyadri_utils::channel::JobReceiver;
use std::{
    sync::Arc,
...
impl IbdFlow {
    ...
    async fn sync_and_validate_pruning_proof(&mut self, staging: &ConsensusProxy, relay_block: &Block) -> Result<Hash, ProtocolError> {
        ...
        // blocks headers, and sahyadri_consensus data, which are required to verify the pruning
    ...
    async fn validate_staging_timestamps(
        ...
        // https://github.com/sahyadrinet/research/issues/3#issuecomment-895243792
    ...
    async fn sync_missing_trusted_bodies_no_headers(
        ...
        for chunk in iter {
            ...
            for &hash in chunk.iter() {
                ...
                // TODO (relaxed): sending sahyadri_consensus data may be redundant, especially when the headers were already verified.
                // Consider sending empty sahyadri_consensus data, simplifying a great deal. The result should be the same -
