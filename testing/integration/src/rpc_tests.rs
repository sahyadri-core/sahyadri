...
use sahyadri_consensus::params::SIMNET_GENESIS;
use sahyadri_consensus_core::{constants::MAX_KANA, header::Header, subnets::SubnetworkId, tx::Transaction};
...
use sahyadri_utils::{fd_budget, networking::ContextualNetAddress};
...
/// `cargo test --release --package sahyadri-testing-integration --lib -- rpc_tests::sanity_test`
#[tokio::test]
async fn sanity_test() {
    ...
    for op in SahyadridPayloadOps::iter() {
        ...
        let task: JoinHandle<()> = match op {
            ...
            SahyadridPayloadOps::GetBlockTemplate => {
                tst!(op, "see SubmitBlock")
            }

            SahyadridPayloadOps::GetCurrentBlockColor => {
                tst!(op, "see SubmitBlock")
            }

            SahyadridPayloadOps::GetCurrentNetwork => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetMempoolEntries => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            SahyadridPayloadOps::GetConnectedPeerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::Unban => {
                tst!(op, "see Ban")
            }

            SahyadridPayloadOps::SubmitTransaction => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::SubmitTransactionReplacement => {
                let rpc_client = client.clone();
                tst!(op, {
                    // Build an erroneous transaction...
                    let transaction = Transaction::new(0, vec![], vec![], 0, SubnetworkId::default(), 0, vec![]);
            ...
            SahyadridPayloadOps::GetSubnetwork => {
                let rpc_client = client.clone();
                tst!(op, {
                    let result =
            ...
            SahyadridPayloadOps::GetVirtualChainFromBlock => {
                tst!(op, "see SubmitBlock")
            }

            SahyadridPayloadOps::GetBlockCount => {
                tst!(op, "see SubmitBlock")
            }

            SahyadridPayloadOps::GetSahyadriDagInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sahyadri_dag_info_call(None, GetSahyadriDagInfoRequest {}).await.unwrap();
                    assert_eq!(response.network, network_id);
                })
            }

            SahyadridPayloadOps::ResolveFinalityConflict => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            SahyadridPayloadOps::GetHeaders => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            SahyadridPayloadOps::GetBalanceByAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            SahyadridPayloadOps::GetBalancesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetSinkBlueScore => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sink_blue_score_call(None, GetSinkBlueScoreRequest {}).await.unwrap();
            ...
            SahyadridPayloadOps::EstimateNetworkHashesPerSecond => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            SahyadridPayloadOps::GetMempoolEntriesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::PubKey, &[0u8; 32])];
                    let response = rpc_client
            ...
            SahyadridPayloadOps::GetCoinSupply => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_coin_supply_call(None, GetCoinSupplyRequest {}).await.unwrap();
                    assert_eq!(response.circulating_kana, 0);
                    assert_eq!(response.max_kana, MAX_KANA);
                })
            }

            SahyadridPayloadOps::Ping => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetConnections => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetMetrics => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetSystemInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetServerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetSyncStatus => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetDaaScoreTimestampEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetFeeEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetFeeEstimateExperimental => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SahyadridPayloadOps::GetUtxoReturnAddress => {
                ...
                tst!(op, {
                    ...
                    assert!(results.is_err_and(|err| {
                        match err {
                            sahyadri_rpc_core::RpcError::General(msg) => {
            ...
            SahyadridPayloadOps::GetVirtualChainFromBlockV2 => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            SahyadridPayloadOps::NotifyBlockAdded => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SahyadridPayloadOps::NotifyNewBlockTemplate => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SahyadridPayloadOps::NotifyFinalityConflict => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SahyadridPayloadOps::NotifyVirtualChainChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
                    rpc_client
