...
use sahyadri_consensus::params::SIMNET_PARAMS;
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  sahyadri_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut sahyadrid1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = sahyadrid1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut sahyadrid2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = sahyadrid2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut sahyadrid1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut sahyadrid2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = sahyadrid1.start().await;
    let rpc_client2 = sahyadrid2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", sahyadrid1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(sahyadrid1.network.into(), sahyadri_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let dag_info = rpc_client2.get_sahyadri_dag_info().await.unwrap();
    ...
    let vc = rpc_client2
        .get_virtual_chain_from_block(
            sahyadri_consensus::params::SIMNET_GENESIS.hash, //
            true,
            None,
        )
        .await
...
async fn daemon_utxos_propagation_test() {
    ...
    sahyadri_core::log::try_init_logger(
        "INFO,sahyadri_testing_integration=trace,sahyadri_notify=debug,sahyadri_rpc_core=debug,sahyadri_grpc_client=debug",
    );
    ...
    // Connect sahyadrid2 to sahyadrid1
    rpc_client2.add_peer(format!("127.0.0.1:{}", sahyadrid1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    // Check that acceptance data contains the expected coinbase tx ids
    let vc = rpc_client2.get_virtual_chain_from_block(sahyadri_consensus::params::SIMNET_GENESIS.hash, true, None).await.unwrap();
    assert_eq!(vc.removed_chain_block_hashes.len(), 0);
    ...
    let mut clients = vec![ListeningClient::connect(&sahyadrid2).await, ListeningClient::connect(&sahyadrid1).await];
...
async fn daemon_cleaning_test() {
    ...
    {
        let total_fd_limit = 10;
        let mut sahyadrid1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = sahyadrid1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = sahyadrid1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&sahyadrid1.core);
