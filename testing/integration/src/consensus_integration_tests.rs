...
use sahyadri_consensus::config::{Config, ConfigBuilder};
use sahyadri_consensus::consensus::factory::Factory as ConsensusFactory;
use sahyadri_consensus::consensus::test_consensus::{TestConsensus, TestConsensusFactory};
...
use sahyadri_consensus::pipeline::monitor::ConsensusMonitor;
use sahyadri_consensus::processes::reachability::tests::{DagBlock, DagBuilder, StoreValidationExtensions};
use sahyadri_consensus::processes::window::{WindowManager, WindowType};
use sahyadri_consensus_core::api::args::TransactionValidationArgs;
use sahyadri_consensus_core::api::{BlockValidationFutures, ConsensusApi};
use sahyadri_consensus_core::block::Block;
use sahyadri_consensus_core::blockhash::{self, new_unique};
use sahyadri_consensus_core::blockstatus::BlockStatus;
use sahyadri_consensus_core::coinbase::MinerData;
use sahyadri_consensus_core::constants::{BLOCK_VERSION, KANA_PER_SAHYADRI, TRANSIENT_BYTE_TO_MASS_FACTOR};
use sahyadri_consensus_core::errors::block::{BlockProcessResult, RuleError};
...
use sahyadri_rpc_core::RpcHeader;
...
#[derive(Serialize, Deserialize, Debug)]
struct SahyadriConsensusTestDag {
    #[serde(rename = "K")]
    k: SahyadriConsensusKType,

    #[serde(rename = "GenesisID")]
    genesis_id: String,

    #[serde(rename = "Blocks")]
    blocks: Vec<SahyadriConsensusTestBlock>,
}

#[derive(Serialize, Deserialize, Debug)]
struct SahyadriConsensusTestBlock {
    #[serde(rename = "ID")]
    id: String,
...
#[tokio::test]
async fn sahyadri_consensus_test() {
    init_allocator_with_default_settings();
    ...
    for path_str in path_strings.iter() {
        ...
        let test: SahyadriConsensusTestDag = serde_json::from_reader(reader).unwrap();

        let config = ConfigBuilder::new(MAINNET_PARAMS)
            .skip_proof_of_work()
            .edit_consensus_params(|p| {
                p.genesis.hash = string_to_hash(&test.genesis_id);
                p.sahyadri_consensus_k = test.k;
        ...
        for block in test.blocks {
            ...
            let block_id = string_to_hash(&block.id);
            let output_sahyadri_consensus_data = sahyadri_consensus_store.get_data(block_id).unwrap();

            assert_eq!(
                output_sahyadri_consensus_data.selected_parent,
            ...
            assert_eq!(
                output_sahyadri_consensus_data.mergeset_reds.to_vec(),
            ...
            assert_eq!(
                output_sahyadri_consensus_data.mergeset_blues.to_vec(),
            ...
            assert_eq!(output_sahyadri_consensus_data.blue_score, block.score, "blue score assertion failed for {}", block.id,);
        }
...
async fn block_window_test() {
    ...
    for test_block in test_blocks {
        ...
        let window = consensus
            .window_manager()
            .block_window(&consensus.sahyadri_consensus_store().get_data(block_id).unwrap(), WindowType::VaryingWindow(10))
            .unwrap()
            .blocks
            .clone();
...
async fn bounded_merge_depth_test() {
    ...
    let config = ConfigBuilder::new(DEVNET_PARAMS)
        .skip_proof_of_work()
        .edit_consensus_params(|p| {
            p.sahyadri_consensus_k = 5;
            p.merge_depth = 7;
        })
        .build();

    assert!((config.sahyadri_consensus_k() as u64) < config.merge_depth(), "K must be smaller than merge depth for this test to run");
    ...
    for i in 0..config.sahyadri_consensus_k() {
...
async fn difficulty_test() {
    ...
    async fn add_block(consensus: &TestConsensus, block_time: Option<u64>, parents: Vec<Hash>) -> Header {
        let selected_parent = consensus.sahyadri_consensus_manager().find_selected_parent(parents.iter().copied());
    ...
    fn past_median_time(consensus: &TestConsensus, parents: &[Hash]) -> u64 {
        let sahyadri_consensus_data = consensus.sahyadri_consensus_manager().sahyadri_consensus(parents);
        consensus.window_manager().calc_past_median_time(&sahyadri_consensus_data).unwrap().0
    }
    ...
    fn full_window_bits(consensus: &TestConsensus, hash: Hash) -> u32 {
        ...
        let sahyadri_consensus_data = &consensus.sahyadri_consensus_store().get_data(hash).unwrap();
        let window = consensus.window_manager().block_window(sahyadri_consensus_data, WindowType::VaryingWindow(window_size)).unwrap();
        assert_eq!(window.blocks.len(), window_size);
        let daa_window = consensus.window_manager().calc_daa_window(sahyadri_consensus_data, window);
        consensus.window_manager().calculate_difficulty_bits(sahyadri_consensus_data, &daa_window)
    }
...
async fn staging_consensus_test() {
    ...
    let db_tempdir = get_sahyadri_tempdir();
    let db_path = db_tempdir.path().to_owned();
    let consensus_db_dir = db_path.join("consensus");
    let meta_db_dir = db_path.join("meta");

    let meta_db = sahyadri_database::prelude::ConnBuilder::default().with_db_path(meta_db_dir).with_files_limit(5).build().unwrap();
    ...
    let consensus_factory = Arc::new(ConsensusFactory::new(
        ...
        sahyadri_database::prelude::RocksDbPreset::Default,
        None,
        None,
    ));
