...
use sahyadri_notify::{events::EventType, scope::Scope, subscription::Command};
...
impl ListeningClient {
    pub async fn connect(sahyadrid: &Daemon) -> Self {
        let client = sahyadrid.new_multi_listener_client().await;
