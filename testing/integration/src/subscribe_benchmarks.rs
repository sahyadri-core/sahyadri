...
use sahyadri_core::{info, task::tick::TickService, trace};
...
async fn utxos_changed_subscriptions_sanity_check() {
    ...
    // Fold-up
    //
    sahyadri_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    sahyadri_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    sahyadri_core::warn!("Waiting for the daemon to exit...");
