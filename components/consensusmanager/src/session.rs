...
impl ConsensusSessionOwned {
    ...
    pub fn get_account_balance(&self, address: &sahyadri_addresses::Address) -> Option<u64> {
    ...
    pub async fn async_get_sahyadri_consensus_data(&self, hash: Hash) -> ConsensusResult<ExternalSahyadriConsensusData> {
        self.clone().spawn_blocking(move |c| c.get_sahyadri_consensus_data(hash)).await
    }
