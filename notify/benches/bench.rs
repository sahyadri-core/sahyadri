...
use sahyadri_notify::{address::tracker::Indexes, subscription::context::SubscriptionContext};

fn create_addresses(count: usize) -> Vec<Address> {
    (0..count)
        .map(|i| Address::new(Prefix::Mainnet, sahyadri_addresses::Version::PubKey, &Uint256::from_u64(i as u64).to_le_bytes()))
        .collect()
}
...
// `cargo bench --package sahyadri-notify --bench bench`
