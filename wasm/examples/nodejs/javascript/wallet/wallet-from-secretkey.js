...
const fs = require('fs');
const sahyadri = require('../../../../nodejs/sahyadri-dev');
const {
    Wallet, setDefaultStorageFolder,
    AccountKind, Mnemonic, Resolver,
    sahyadriToKana,
    kanaToSahyadriString,
    Address
} = sahyadri;
...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: kanaToSahyadriString(value(tx)||0)
            });
    ...
    try {
        ...
        // Send sahyadri to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            // @ts-ignore
            accountId: firstAccount.accountId,
            priorityFeeKana: sahyadriToKana("0.001"),
            destination:[{
                // @ts-ignore
                address: firstAccount.changeAddress,
                amount: sahyadriToKana("1.567")
            }]
        });
        ...
        // Transfer sahyadri between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountKana: sahyadriToKana("2.456"),
        });
