...
- [**Rust** documentation](https://docs.rs/sahyadri-wasm/latest/sahyadri_wasm/index.html)
- [**TypeScript** documentation](https://sahyadri.aspectron.org/docs/)
...
Included documentation in the release can be accessed by loading the `docs/sahyadri/index.html` 
...
before loading the WASM32 library. The compatible WebSocket library is [WebSocket](https://www.npmjs.com/package/websocket) and is included in the `sahyadri` NPM package. `sahyadri` package is a wrapper around `sahyadri-wasm` that imports and installs this WebSocket shim in the `globalThis` object and then re-exports `sahyadri-wasm` exports.
...
<html>
    <head>
        <script type="module">
            import * as sahyadri from './sahyadri/sahyadri-wasm.js';
            (async () => {
                await sahyadri.default('./sahyadri/sahyadri-wasm_bg.wasm');
