...
pub(crate) struct Kana(u64);

#[derive(Clone, Copy)]
struct Sahyadri(f64);

impl Debug for Sahyadri {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let kana: Kana = self.into();
        write!(f, "{}", kana.0)
    }
}

impl From<Sahyadri> for Kana {
    fn from(sahyadri: Sahyadri) -> Self {
        Kana(sahyadri_to_kana(sahyadri.0))
    }
}
...
impl FeesExpected {
    fn sender<T: Into<Kana>>(v: T) -> Self {
        let kana: Kana = v.into();
        FeesExpected::Sender(kana.0)
    }
    fn receiver<T: Into<Kana>>(v: T) -> Self {
        let kana: Kana = v.into();
        FeesExpected::Receiver(kana.0)
    }
}
...
trait PendingTransactionExtension {
    ...
    fn expect<KANA>(self, expected: &Expected<KANA>) -> Self
    where
        KANA: Into<Kana> + Debug + Copy;
...
impl FeesExtension for Fees {
    fn sender<T: Into<Kana>>(v: T) -> Self {
        let kana: Kana = v.into();
        Fees::SenderPays(kana.0)
    }
    fn receiver<T: Into<Kana>>(v: T) -> Self {
        let kana: Kana = v.into();
        Fees::ReceiverPays(kana.0)
    }
}
...
#[derive(Debug)]
pub(crate) struct Expected<KANA: Into<Kana>> {
    is_final: bool,
    input_count: usize,
    aggregate_input_value: KANA,
    output_count: usize,
...
fn expect<KANA>(pt: &PendingTransaction, expected: &Expected<KANA>)
where
    KANA: Into<Kana> + Debug + Copy,
{
    let network_params = pt.generator().network_params();
    ...
    let expected_aggregate_input_value: Kana = expected.aggregate_input_value.into();
...
impl Harness {
    ...
    pub fn fetch<KANA>(self: &Rc<Self>, expected: &Expected<KANA>) -> Rc<Self>
    where
        KANA: Into<Kana> + Debug + Copy,
    {
        if DISPLAY_LOGS {
    ...
    pub fn drain<KANA>(self: &Rc<Self>, count: usize, expected: &Expected<KANA>) -> Rc<Self>
    where
        KANA: Into<Kana> + Debug + Copy,
    {
        for _n in 0..count {
    ...
    F: FnOnce(NetworkType) -> Address + Clone,
{
    let outputs = outputs
        .iter()
        .map(|(address, amount)| {
            let kana: Kana = (*amount).clone().into();
            (address.clone()(network_id.into()), kana.0)
        })
        .collect::<Vec<_>>();
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], None, Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Sahyadri(20.0),
            output_count: 1,
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Sahyadri(5.0)),
        [(output_address, Sahyadri(190_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .validate()
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Sahyadri(total - 10.0))];
    generator(test_network_id(), &inputs, &[], None, Fees::sender(Sahyadri(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Sahyadri(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], None, Fees::sender(Sahyadri(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Sahyadri(5.0)),
        [(output_address, Sahyadri(1.0)), (output_address, Sahyadri(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Sahyadri(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Sahyadri(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Sahyadri(5.0)),
        [(output_address, Sahyadri(10.0)), (output_address, Sahyadri(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    ...
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Sahyadri(0.0)), [(output_address, Sahyadri(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Sahyadri(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Kana(999_99886576),
            output_count: 2,
            // priority_fees: FeesExpected::sender(Sahyadri(5.0)),
            priority_fees: FeesExpected::sender(Sahyadri(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[],
        None,
        Fees::receiver(Sahyadri(5.0)),
        // [(output_address, Sahyadri(100.0))].as_slice(),
        [(output_address, Sahyadri(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Sahyadri(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Sahyadri(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: true,
        input_count: 2,
        aggregate_input_value: Kana(99_99886576),
        output_count: 1,
...
fn test_generator_inputs_32k_outputs_2_fees_exclude() -> Result<()> {
    ...
    generator(
        ...
        &[],
        None,
        Fees::sender(Sahyadri(10_000.0)),
        [(output_address, Sahyadri(f * 32_747.0 - 10_001.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .accumulate(379)
...
fn test_generator_fan_out_1() -> Result<()> {
    ...
    //     1.00000000,
    //     87.23579967,
    // ], &[], None, Fees::sender(Sahyadri(1.0)), [
    //     (output_address, Sahyadri(0.20000000)),
    //     (output_address, Sahyadri(0.25000000)),
    //     (output_address, Sahyadri(0.21000000)),
    // ].as_slice())
    //     .unwrap()
    //     .harness()
    //     // .accumulate(1)
    ...
    //         is_final: true,
    //         input_count: 2,
    //         aggregate_input_value: Sahyadri(1.00000000 + 87.23579967),
    //         output_count: 4,
    //         priority_fees: FeesExpected::receiver(Sahyadri(1.0)),
    //         // priority_fees: FeesExpected::None,
    //     })
    //     .finalize();
