...
use sahyadri_wrpc_client::SahyadriRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<SahyadriRpcClient>> {
        self.rpc_api().clone().downcast_arc::<SahyadriRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to sahyadrid: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");
