...
declare! {
     ...
     * with the default filename `sahyadri`.
     * 
     * @category Wallet API
     */
...
declare! {
    ...
    export type IAccountsCreateRequest = {
        ...
        walletSecret: string;
        type: "sahyadri-keypair-standard";
        accountName:string;
        prvKeyDataId:string;
        paymentSecret?:string;
        ecdsa?:boolean;
...
try_from! (args: IAccountsCreateRequest, AccountsCreateRequest, {
    ...
    let account_create_args = match kind.as_str() {
        ...
        _ => {
            return Err(Error::custom("only BIP32/sahyadri-keypair-standard accounts are currently supported"));
        }
    };
...
declare! {
    ...
    export interface IAccountsSendRequest {
        ...
        paymentSecret? : string;
        /**
         * Fee rate in kana per 1 gram of mass.
         */
        feeRate? : number;
        /**
         * Priority fee.
         */
        priorityFeeKana? : IFees | bigint;
        /**
         * 
         */
...
try_from! ( args: IAccountsSendRequest, AccountsSendRequest, {
    ...
    let priority_fee_kana = args.get::<IFees>("priorityFeeKana")?.try_into()?;
    ...
    Ok(AccountsSendRequest { account_id, wallet_secret, payment_secret, fee_rate, priority_fee_kana, destination, payload })
});
...
try_from! ( args: IAccountsTransferRequest, AccountsTransferRequest, {
    ...
    let payment_secret = args.try_get_secret("paymentSecret")?;
    let fee_rate = args.get_f64("feeRate").ok();
    let priority_fee_kana = args.try_get::<IFees>("priorityFeeKana")?.map(Fees::try_from).transpose()?;
    let transfer_amount_kana = args.get_u64("transferAmountKana")?;
...
try_from! ( args: IAccountsEstimateRequest, AccountsEstimateRequest, {
    ...
    Ok(AccountsEstimateRequest { account_id, fee_rate, priority_fee_kana, destination, payload })
});
...
declare! {
     ...
     * 
     * The default revealFeeKana is 100_000 kana.
     *  
     * @category Wallet API
     */
    export interface IAccountsCommitRevealRequest {
        ...
        walletSecret : string;
        commitAmountKana : bigint;
        paymentSecret? : string;
        feeRate? : number;
        revealFeeKana : bigint;
        payload? : Uint8Array | HexString;
    }
    "#,
}
...
try_from! ( args: IAccountsCommitRevealRequest, AccountsCommitRevealRequest, {
    ...
    let wallet_secret = args.get_secret("walletSecret")?;
    let payment_secret = args.try_get_secret("paymentSecret")?;
    let commit_amount_kana = args.get_u64("commitAmountKana")?;
    let fee_rate = args.get_f64("feeRate").ok();

    let reveal_fee_kana = args.get_u64("revealFeeKana")?;
