//!
//! Legacy (KDX, sahyadrinet.io Web Wallet) account implementation
//!
use std::sync::OnceLock;

use crate::account::{AsLegacyAccount, Inner};
use crate::derivation::{AddressDerivationManager, AddressDerivationManagerTrait, AddressDerivationMeta};
use crate::imports::*;
use sahyadri_bip32::{ExtendedPrivateKey, Prefix, SecretKey};

const CACHE_ADDRESS_OFFSET: u32 = 2048;

pub const LEGACY_ACCOUNT_KIND: &str = "sahyadri-legacy-standard";
...
impl Factory for Ctor {
    ...
    fn description(&self) -> String {
        "Sahyadri Legacy Account (KDX, sahyadrinet.io Web Wallet)".to_string()
    }
...
pub struct Legacy {
    ...
    derivation: OnceLock<Arc<AddressDerivationManager>>,
...
impl Legacy {
    pub async fn try_new(wallet: &Arc<Wallet>, name: Option<String>, prv_key_data_id: PrvKeyDataId) -> Result<Self> {
        ...
        let address_derivation_indexes = Default::default();
    ...
    pub async fn initialize_derivation(
        ...
        let derivation = crate::derivation::AddressDerivationManager::create_legacy_pubkey_managers(
            &self.inner.wallet,
            account_index,
            self.address_derivation_indexes.clone(),
            &xprv,
        )?;

        self.derivation.set(derivation).map_err(|_| "already initialized")?;

        let d = self.derivation.get().unwrap();
        let m = d.receive_address_manager();
        m.get_range(0..(m.index() + CACHE_ADDRESS_OFFSET))?;
        let m = d.change_address_manager();
        m.get_range(0..(m.index() + CACHE_ADDRESS_OFFSET))?;
    ...
    fn derivation(&self) -> &Arc<AddressDerivationManager> {
        self.derivation.get().expect("derivation not initialized")
    }
}
...
impl Account for Legacy {
    ...
    fn account_addresses(&self) -> Result<Vec<Address>> {
        ...
        let meta = d.address_derivation_meta();
        let receive = meta.receive();
        let change = meta.change();
        let mut addresses = d.receive_address_manager().get_range_with_args(0..receive, false)?;
        let change_addresses = d.change_address_manager().get_range_with_args(0..change, false)?;
...
impl DerivationCapableAccount for Legacy {
    fn derivation(&self) -> Arc<dyn AddressDerivationManagerTrait> {
        self.derivation.get().expect("derivation not initialized").clone()
    }
