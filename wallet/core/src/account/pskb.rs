...
//! (Partial Signed Sahyadri Transaction Bundles).
//!
...
pub fn pskt_to_pending_transaction(
    ...
    let output: &Vec<sahyadri_consensus_core::tx::TransactionOutput> = &signed_tx.outputs;
    if output.is_empty() {
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        sahyadri_wallet_core::tx::DataKind::Final,
    )?;
...
pub enum CommitRevealBatchKind {
    ...
    Parameterized { address: Address, commit_amount_kana: u64 },
}
...
pub async fn commit_reveal_batch_bundle(
    ...
    let conf: BundleCommitRevealConfig = match batch_config {
        ...
        CommitRevealBatchKind::Parameterized { address, commit_amount_kana } => {
            ...
            let amt_reveal: u64 = commit_amount_kana - reveal_fee_kana;

            BundleCommitRevealConfig {
                address_commit: lock_address.clone(),
                addresses_reveal: vec![address.clone()],
                commit_destination: PaymentDestination::from(PaymentOutput::new(lock_address, commit_amount_kana)),
                redeem_script,
