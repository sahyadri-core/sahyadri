//!
//! Sahyadri value formatting and parsing utilities.
//!
...
pub fn try_sahyadri_str_to_kana<S: Into<String>>(s: S) -> Result<Option<u64>> {
    let s: String = s.into();
    let amount = s.trim();
    if amount.is_empty() {
        return Ok(None);
...
pub fn try_sahyadri_str_to_kana_i64<S: Into<String>>(s: S) -> Result<Option<i64>> {
    let s: String = s.into();
    let amount = s.trim();
    if amount.is_empty() {
        return Ok(None);
    ...
    let amount = amount.parse::<f64>()? * KANA_PER_SAHYADRI as f64;
...
#[inline]
pub fn kana_to_sahyadri_string_with_trailing_zeroes_and_suffix(kana: u64, network_type: &NetworkType) -> String {
    let kas = kana_to_sahyadri_string_with_trailing_zeroes(kana);
    let suffix = sahyadri_suffix(network_type);
    format!("{kas} {suffix}")
}
...
fn str_to_kana(amount: &str) -> Result<u64> {
    let Some(dot_idx) = amount.find('.') else {
        return Ok(amount.parse::<u64>()? * KANA_PER_SAHYADRI);
    };
    let integer = amount[..dot_idx].parse::<u64>()? * KANA_PER_SAHYADRI;
