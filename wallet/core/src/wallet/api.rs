...
use crate::tx::Fees;
use sahyadri_rpc_core::RpcFeeEstimate;
...
impl WalletApi for super::Wallet {
    ...
    async fn wallet_open_call(self: Arc<Self>, request: WalletOpenRequest) -> Result<WalletOpenResponse> {
        ...
        Ok(WalletOpenResponse { account_descriptors: Some(account_descriptors) })
    }
    ...
    async fn accounts_send_call(self: Arc<Self>, request: AccountsSendRequest) -> Result<AccountsSendResponse> {
        let AccountsSendRequest { account_id, wallet_secret, payment_secret, destination, fee_rate, priority_fee_kana, payload } =
            request;
        ...
        let (generator_summary, transaction_ids) =
            account.send(destination, fee_rate, priority_fee_kana, payload, wallet_secret, payment_secret, &abortable, None).await?;
    ...
    async fn accounts_transfer_call(self: Arc<Self>, request: AccountsTransferRequest) -> Result<AccountsTransferResponse> {
        ...
        let (generator_summary, transaction_ids) = source_account
            .transfer(
                ...
                transfer_amount_kana,
                fee_rate,
                priority_fee_kana.unwrap_or(Fees::SenderPays(0)),
                wallet_secret,
                payment_secret,
                &abortable,
                None,
    ...
    async fn accounts_estimate_call(self: Arc<Self>, request: AccountsEstimateRequest) -> Result<AccountsEstimateResponse> {
        let AccountsEstimateRequest { account_id, destination, fee_rate, priority_fee_kana, payload } = request;
        ...
        let result = account.estimate(destination, fee_rate, priority_fee_kana, payload, &abortable).await;
