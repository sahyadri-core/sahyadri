//!
//! # Sahyadri wallet runtime implementation.
//!
//! This module contains a Rust implementation of the Sahyadri wallet that
...
use sahyadri_bip32::{ExtendedKey, Language, Mnemonic, Prefix as KeyPrefix, WordCount};
use sahyadri_notify::{
    listener::ListenerId,
...
struct Inner {
    ...
    pub(crate) wallet_secret: Mutex<Option<Secret>>,
    pub(crate) payment_secret: Mutex<Option<Secret>>,
}
...
impl Wallet {
    ...
    pub fn secret(&self) -> Option<Secret> {
        self.inner.wallet_secret.lock().unwrap().clone()
    }
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<SahyadriRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<SahyadriRpcClient>().ok())
    }

    pub fn wrpc_client(&self) -> Arc<SahyadriRpcClient> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<SahyadriRpcClient>().ok()).unwrap()
    }
    ...
    pub fn address_prefix(&self) -> Result<sahyadri_addresses::Prefix> {
    ...
    pub async fn import_sahyadriwallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        ...
        let prefix = file.xpublic_key.split_at(sahyadri_bip32::Prefix::LENGTH).0;
        let prefix = sahyadri_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_sahyadriwallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_sahyadriwallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        let prefix = first_pub_key.split_at(sahyadri_bip32::Prefix::LENGTH).0;
        let prefix = sahyadri_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_sahyadriwallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        all_pub_keys.sort_unstable_by(|left, right| {
            left.split_at(sahyadri_bip32::Prefix::LENGTH).1.cmp(right.split_at(sahyadri_bip32::Prefix::LENGTH).1)
        });
    ...
    pub async fn import_multisig_with_mnemonic(
        ...
        let mut additional_xpub_keys = additional_xpub_keys
            ...
            .collect::<Result<Vec<_>, sahyadri_bip32::Error>>()?;
...
mod test {
    ...
    use sahyadri_bip32::{ChildNumber, ExtendedPrivateKey, SecretKey};
    use sahyadri_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use sahyadri_consensus_wasm::{sign_transaction, SignableTransaction, Transaction, TransactionInput, TransactionOutput};
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let info = rpc_api.get_sahyadri_dag_info().await?;
        ...
        let address = Address::try_from("sahyadritest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd")?;
        ...
        let to_address = Address::try_from("sahyadritest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c")?;
        ...
        let derivation_path =
            gen1::WalletDerivationManager::build_derivate_path(false, 0, None, Some(sahyadri_bip32::AddressType::Receive))?;
