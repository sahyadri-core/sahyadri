//!
//! API trait for interfacing with the Sahyadri wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `sahyadri`.
    ///
    ...
    /// applications such as KDX and sahyadrinet.io web wallet using a deprecated derivation path
