...
use sahyadri_bip32::Error as BIP32Error;
use sahyadri_consensus_core::sign::Error as CoreSignError;
use sahyadri_rpc_core::RpcError as SahyadriRpcError;
use sahyadri_wrpc_client::error::Error as SahyadriWorkflowRpcError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] sahyadri_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    SahyadriRpcClientResult(#[from] SahyadriRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    SahyadriWorkflowRpcError(#[from] SahyadriWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] sahyadri_consensus_core::network::NetworkTypeError),
    ...
    #[error(transparent)]
    Address(#[from] sahyadri_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] sahyadri_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusWasm(#[from] sahyadri_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] sahyadri_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] sahyadri_txscript_errors::TxScriptError),
    ...
    #[error(transparent)]
    Metrics(#[from] sahyadri_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Pskt(#[from] sahyadri_wallet_pskt::error::Error),
