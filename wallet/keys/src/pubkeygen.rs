...
use crate::imports::*;
use sahyadri_addresses::AddressArrayT;
...
///
/// Please note that in Sahyadri master public keys use `kpub` prefix.
///
...
impl PublicKeyGenerator {
    ...
    pub fn receive_pubkeys(&self, mut start: u32, mut end: u32) -> Result<PublicKeyArrayT> {
        ...
        Ok(Array::from_iter(pubkeys.into_iter().map(|pk| JsValue::from(PublicKey::from(&pk.0[..])))).unchecked_into())
    }
    ...
    pub fn receive_pubkey(&self, index: u32) -> Result<PublicKey> {
        Ok(PublicKey::from(&self.hd_wallet.receive_pubkey_manager().derive_pubkey(index)?.0[..]))
    }
    ...
    pub fn receive_pubkeys_as_strings(&self, mut start: u32, mut end: u32) -> Result<StringArray> {
        ...
        let pubkeys = self.hd_wallet.receive_pubkey_manager().derive_pubkey_range(start..end)?;
        Ok(Array::from_iter(pubkeys.into_iter().map(|pk| JsValue::from(PublicKey::from(&pk.0[..]).to_string()))).unchecked_into())
    }
    ...
    pub fn receive_addresses(&self, networkType: &NetworkTypeT, mut start: u32, mut end: u32) -> Result<AddressArrayT> {
        ...
        let pubkeys = self.hd_wallet.receive_pubkey_manager().derive_pubkey_range(start..end)?;
        let addresses =
            pubkeys.into_iter().map(|pk| PublicKey::from(&pk.0[..]).to_address(network_type)).collect::<Result<Vec<Address>>>()?;
    ...
    pub fn receive_address(&self, networkType: &NetworkTypeT, index: u32) -> Result<Address> {
        PublicKey::from(&self.hd_wallet.receive_pubkey_manager().derive_pubkey(index)?.0[..]).to_address(networkType.try_into()?)
    }
    ...
    pub fn change_pubkey(&self, index: u32) -> Result<PublicKey> {
        Ok(PublicKey::from(&self.hd_wallet.change_pubkey_manager().derive_pubkey(index)?.0[..]))
    }
    ...
    pub fn change_pubkey_as_string(&self, index: u32) -> Result<String> {
        Ok(faster_hex::hex_string(&self.hd_wallet.change_pubkey_manager().derive_pubkey(index)?.0))
    }
    ...
    pub fn change_address(&self, networkType: &NetworkTypeT, index: u32) -> Result<Address> {
        PublicKey::from(&self.hd_wallet.change_pubkey_manager().derive_pubkey(index)?.0[..]).to_address(networkType.try_into()?)
    }
