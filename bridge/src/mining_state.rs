...
pub struct MiningState {
    ...
    stratum_diff: Arc<Mutex<Option<SahyadriDiff>>>,
    max_jobs: u16,
    last_header: Arc<Mutex<Option<sahyadri_consensus_core::header::Header>>>, // Track previous header for change logging
}
...
impl MiningState {
    ...
    pub fn stratum_diff(&self) -> Option<SahyadriDiff> {
        self.stratum_diff.lock().clone()
    }

    /// Set stratum difficulty
    pub fn set_stratum_diff(&self, diff: SahyadriDiff) {
    ...
    /// Get last header
    pub fn get_last_header(&self) -> Option<sahyadri_consensus_core::header::Header> {
        self.last_header.lock().clone()
    }

    /// Set last header
    pub fn set_last_header(&self, header: sahyadri_consensus_core::header::Header) {
