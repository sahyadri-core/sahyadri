...
pub fn init_metrics() {
    ...
    BLOCK_ACCEPTED_COUNTER.get_or_init(|| {
        register_counter_vec!(
            ...
            "Number of blocks accepted by the connected Sahyadri node (may later be red)",
            WORKER_LABELS
        )
        .unwrap()
    });
...
pub fn record_balances(instance_id: &str, balances: &[(String, u64)]) {
    if let Some(gauge) = BALANCE_GAUGE.get() {
        for (address, balance) in balances {
            // Convert from kana to CSM (divide by 100000000)
...
async fn get_config_json() -> String {
        ...
        && let Some(doc) = docs.first()
    {
        ...
        if let Some(addr) = doc["sahyadrid_address"].as_str() {
            config.insert("sahyadrid_address".to_string(), serde_json::Value::String(addr.to_string()));
        }
...
async fn update_config_from_json(json_body: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    out_str.push_str("# This file configures the stratum bridge that connects miners to the Sahyadri node\n\n");
    ...
    if let Some(addr) = config.get("sahyadrid_address").and_then(|v| v.as_str()) {
        out_str.push_str("# Sahyadri node gRPC address (format: \"HOST:PORT\" or \"grpc://HOST:PORT\")\n");
        out_str.push_str(&format!("sahyadrid_address: {}\n\n", addr));
    }
