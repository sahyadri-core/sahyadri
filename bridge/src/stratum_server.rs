use crate::{
    ...
    sahyadriapi::SahyadriApi,
    share_handler::{SahyadriApiTrait, ShareHandler},
...
pub async fn listen_and_serve<T: SahyadriApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    sahyadri_api: Arc<T>,
    // Optional: if concrete SahyadriApi is provided, use notification-based listener
    concrete_sahyadri_api: Option<Arc<SahyadriApi>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let authorize_handler = {
        let client_handler = Arc::clone(&client_handler);
        let sahyadri_api = Arc::clone(&sahyadri_api);
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            let client_handler = Arc::clone(&client_handler);
            let sahyadri_api = Arc::clone(&sahyadri_api);
            let ctx_clone = Arc::clone(&ctx);
            let event_clone = event.clone();
            Box::pin(async move {
                handle_authorize(ctx_clone, event_clone, Some(client_handler), Some(sahyadri_api))
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    let submit_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            Box::pin(async move {
                share_handler
                    .handle_submit(ctx_clone, event, sahyadri_api)
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    // If concrete SahyadriApi is provided, use notification-based listener
    // Otherwise, use polling only (fallback for trait objects)
    if let Some(concrete_api) = concrete_sahyadri_api {
        // We have concrete SahyadriApi - use notification-based listener
        let client_handler_cb = Arc::clone(&client_handler);
        let sahyadri_api_cb = Arc::clone(&sahyadri_api);
        ...
        // Call the method directly on Arc<SahyadriApi> (it's an instance method taking Arc<Self>)
        ...
        // No concrete SahyadriApi provided - use polling only
        warn!("Using polling-based block template listener (concrete SahyadriApi not provided, notifications not available)");

        let client_handler_poll = Arc::clone(&client_handler);
        let sahyadri_api_poll = Arc::clone(&sahyadri_api);
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                // Poll for new blocks
                client_handler_poll.new_block_available(Arc::clone(&sahyadri_api_poll)).await;
            }
        });
    }
