    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        // Special-case forwarded node logs (from `tracing_log::LogTracer`) to match sahyadrid style:
        ...
        if self.apply_colors {
            ...
            if message.contains("[ASIC->BRIDGE]") {
                ...
                // Configuration lines - color the label part (e.g., "\tsahyadrid:          value")
...
pub(crate) fn init_tracing(
    ...
    // In inprocess mode, the embedded node primarily uses the `log` crate (via sahyadri_core::* macros).
