...
// sahyadri_pow used inline for PoW validation
...
impl ShareHandler {
    ...
    pub async fn handle_submit(
        ...
        event: JsonRpcEvent,
        sahyadri_api: Arc<dyn SahyadriApiTrait + Send + Sync>,
        ...
        if let Some(Value::String(submitted_identity)) = event.params.first() {
            ...
            let submitted_clean = submitted_address.trim_start_matches("sahyadri:").trim_start_matches("sahyadritest:");
            let authorized_clean = wallet_addr.trim_start_matches("sahyadri:").trim_start_matches("sahyadritest:");
        ...
        loop {
            ...
            // Use sahyadri_pow::State for proper PoW validation
            use sahyadri_pow::State as PowState;
            ...
            if meets_network_target {
                ...
                // The header comes directly from the Sahyadri node via get_block_template_call()
                // We preserve: version, bits, timestamp, all hash fields, parents, scores, etc.
                header_clone.nonce = nonce_val;

                // Verify timestamp is still valid (not too old)
                // Sahyadri typically accepts blocks with timestamps within a reasonable window
                ...
                debug!(
                    ...
                    "Using REAL header from Sahyadri node block template"
                );
                ...
                debug!("{} {}", LogColors::block("[BLOCK]"), "Calling sahyadri_api.submit_block()...");

                // Submit block to node
                let block_submit_result = sahyadri_api.submit_block(block.clone()).await;
                    ...
                    Ok(_response) => {
                        ...
                        tokio::spawn(async move {
                            for _ in 0..BLOCK_CONFIRM_MAX_ATTEMPTS {
                                match sahyadri_api.get_current_block_color(&block_hash_for_confirm).await {
                                    Ok(true) => {
...
// Trait for sahyadri API operations
#[async_trait::async_trait]
pub trait SahyadriApiTrait: Send + Sync {
