...
/// Sahyadri difficulty representation
#[derive(Debug, Clone)]
pub struct SahyadriDiff {
    pub hash_value: f64,
    pub diff_value: f64,
    pub target_value: BigUint,
}

impl Default for SahyadriDiff {
    fn default() -> Self {
...
pub fn diff_to_target_alternative(diff: f64) -> BigUint {
    ...
    // CSM_MAX_TARGET = 2^64 - 1
    const CSM_MAX_TARGET: u64 = 0xFFFFFFFFFFFFFFFF;
    let max_target_big = BigUint::from(CSM_MAX_TARGET);
...
pub fn stratum_difficulty_to_target_sahyadri(stratum_diff: u64) -> BigUint {
    // Handle zero difficulty - return maximum target
    if stratum_diff == 0 {
        const CSM_MAX_TARGET: u64 = 0xFFFFFFFFFFFFFFFF;
        return BigUint::from(CSM_MAX_TARGET) * BigUint::from(1000u64);
    }

    // CSM_MAX_TARGET = 2^64 - 1
    const CSM_MAX_TARGET: u64 = 0xFFFFFFFFFFFFFFFF;
    let base_target = BigUint::from(CSM_MAX_TARGET);
...
/// Based on: https://github.com/tmrlvi/sahyadri-miner/blob/bf361d02a46c580f55f46b5dfa773477634a5753/src/client/stratum.rs#L375
pub fn diff_to_target(diff: f64) -> BigUint {
    ...
    if use_stratum_alt {
        ...
        let stratum_target = stratum_difficulty_to_target_sahyadri(diff_u64);
...
///
/// NOTE: Timestamp is in MILLISECONDS (as per Sahyadri block header format)
/// Pool uses: timestampLE.writeBigUInt64LE(timestamp) which writes u64 in LE
pub fn generate_iceriver_job_params(pre_pow_hash: &sahyadri_hashes::Hash, timestamp: u64) -> String {
...
/// This matches Sahyadri's compact difficulty format
