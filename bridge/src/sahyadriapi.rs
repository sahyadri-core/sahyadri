...
impl SahyadriApi {
    ...
    pub async fn new(address: String, _block_wait_time: Duration, coinbase_tag_suffix: Option<String>) -> Result<Arc<Self>> {
        ...
        debug!("{} {}", LogColors::api("[API]"), LogColors::label("Establishing RPC connection to Sahyadri node:"));
        ...
        // Connect to Sahyadri node with grpc:// prefix, using extended request timeout and reconnection support
        let client = Arc::new(
            ...
            .await
            .context("Failed to connect to Sahyadri node")?,
        );
    ...
    async fn start_stats_thread(self: Arc<Self>) {
        ...
        use sahyadri_rpc_core::{EstimateNetworkHashesPerSecondRequest, GetSahyadriDagInfoRequest};
        ...
        loop {
            ...
            // Get block DAG info
            // GetSahyadriDagInfoRequest is a unit struct, construct directly
            let dag_response = match self.client.get_sahyadri_dag_info_call(None, GetSahyadriDagInfoRequest {}).await {
                Ok(r) => r,
                Err(e) => {
                    warn!("failed to get network hashrate from sahyadri, prom stats will be out of date: {}", e);
                    continue;
                }
            };
    ...
    async fn start_node_status_thread(self: Arc<Self>) {
        ...
        loop {
            ...
            let dag_info_fut = self.client.get_sahyadri_dag_info_call(None, GetSahyadriDagInfoRequest {});
    ...
    pub async fn submit_block(&self, block: Block) -> Result<SubmitBlockResponse> {
        // Use sahyadri_consensus_core::hashing::header::hash() for block hash calculation
        // In Sahyadri, the block hash is the header hash (transactions are represented by hash_merkle_root in header)
        use sahyadri_consensus_core::hashing::header;
        ...
        debug!(
            "{} {}",
            LogColors::api("[API]"),
            LogColors::api(&format!("✓ ===== ATTEMPTING BLOCK SUBMISSION TO CSMPA NODE ===== Hash: {}", block_hash))
        );
        ...
        match &result {
            Ok(response) => {
                ...
                info!(
                    "{} {}",
                    LogColors::api("[API]"),
                    LogColors::block(&format!("===== BLOCK ACCEPTED BY CSMPA NODE ===== Hash: {}", block_hash))
                );
                ...
                tokio::spawn(async move {
                    ...
                    if let Ok(dag_response) = client_clone.get_sahyadri_dag_info_call(None, GetSahyadriDagInfoRequest {}).await {
            ...
            Err(e) => {
                ...
                if error_str.contains("ErrDuplicateBlock") || error_str.contains("duplicate") {
                    warn!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::validation(&format!("===== BLOCK REJECTED BY CSMPA NODE: STALE ===== Hash: {}", block_hash))
                    );
                    warn!(
                        "{} {} {}",
                        LogColors::api("[API]"),
                    ...
                    error!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::error(&format!("===== BLOCK REJECTED BY CSMPA NODE: INVALID ===== Hash: {}", block_hash))
                    );
    ...
    async fn wait_for_sync(&self, verbose: bool) -> Result<()> {
        if verbose {
            debug!("checking sahyadrid sync state");
        }
        ...
        loop {
            match self.client.get_sync_status().await {
                Ok(is_synced) => {
                    if is_synced {
                        if verbose {
                            debug!("sahyadrid synced, starting server");
                        }
                        break;
                    }
                }
            ...
            if verbose {
                warn!("Sahyadri is not synced, waiting for sync before starting bridge");
            }
                ...
                Err(e) => {
                    ...
                    if error_str.contains("Odd number of digits") {
                        return Err(anyhow::anyhow!(
                            "Failed to convert RPC block to Block after {} attempts: {} - This usually indicates a malformed hash field in the block template from the Sahyadri node. The block may have a hash with an odd-length hex string.",
                            max_retries,
                            error_str
                        ));
                    } else {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = api_clone.wait_for_sync(false).await {
                    error!("error checking sahyadrid sync state, attempting reconnect: {}", e);
...
impl SahyadriApiTrait for SahyadriApi {
    async fn get_block_template(
        ...
        SahyadriApi::get_block_template(self, wallet_addr, "", "").await.map_err(|e| {
    ...
    async fn submit_block(
        &self,
        block: Block,
    ) -> Result<sahyadri_rpc_core::SubmitBlockResponse, Box<dyn std::error::Error + Send + Sync>> {
        SahyadriApi::submit_block(self, block)
            .await
    ...
    async fn get_balances_by_addresses(
        ...
        SahyadriApi::get_balances_by_addresses(self, addresses)
            .await
            .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
    }

    async fn get_current_block_color(&self, block_hash: &str) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        SahyadriApi::get_current_block_color(self, block_hash)
            .await
            .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
    }
}
