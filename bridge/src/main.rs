...
use sahyadri_stratum_bridge::log_colors::LogColors;
use sahyadri_stratum_bridge::{BridgeConfig as StratumBridgeConfig, SahyadriApi, listen_and_serve, prom};
use std::ffi::OsString;
use std::path::{Path, PathBuf};
use std::sync::{Arc, OnceLock};
use tracing_subscriber::EnvFilter;

use sahyadrid_lib::args as sahyadrid_args;
...
struct Cli {
    ...
    #[arg(last = true, help = "Sahyadrid arguments (use '--' separator if sahyadrid args start with hyphens)")]
    sahyadrid_args: Vec<String>,
}
...
fn log_bridge_configuration(config: &app_config::BridgeConfig) {
    ...
    tracing::info!("\tsahyadrid:          {} (shared)", config.global.sahyadrid_address);
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        ...
        if node_mode == NodeMode::Inprocess {
            EnvFilter::new("warn,sahyadri_stratum_bridge=info,sahyadri=info,sahyadrid=info,sahyadrid_lib=info,log=info")
        } else {
            EnvFilter::new("warn,sahyadri_stratum_bridge=info")
        }
    });

    // Start in-process node after tracing is initialized so bridge logs (including the stats table)
    // are not filtered out by a tracing subscriber installed by sahyadrid.
    let mut inprocess_node: Option<InProcessNode> = None;
    if node_mode == NodeMode::Inprocess {
        let mut node_args: Vec<String> = cli.sahyadrid_args;

        // Add appdir if not provided in sahyadrid_args
        if !node_args.iter().any(|arg| arg.starts_with("--appdir")) {
            let default_appdir = app_dirs::default_inprocess_sahyadrid_appdir();
            ...
            assert!(cli.appdir.is_none(), "appdir should not be specified both in bridge args and sahyadrid args");
        }

        let mut argv: Vec<OsString> = Vec::with_capacity(node_args.len() + 1);
        argv.push(OsString::from("sahyadrid"));
        argv.extend(node_args.iter().map(OsString::from));
        let args = sahyadrid_args::Args::parse(argv).map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    for (idx, instance_config) in config.instances.iter().enumerate() {
        ...
        let sahyadri_api_clone = Arc::clone(&sahyadri_api);
        ...
        let handle = tokio::spawn(async move {
            ...
            let bridge_config = StratumBridgeConfig {
                ...
                sahyadrid_address: global.sahyadrid_address.clone(),
