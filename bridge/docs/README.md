...
- **External** node (you run `sahyadrid` yourself)
- **In-process** node (the bridge starts `sahyadrid` in the same process)
...
This will show all available bridge options and guidance for sahyadrid arguments.
...
```bash
cargo run -p sahyadri-stratum-bridge --release --bin stratum-bridge -- --config bridge/config.yaml --node-mode external
```
...
**Note:** In-process mode uses a separate app directory by default to avoid RocksDB lock conflicts with an existing `sahyadrid`.
...
- **Username / wallet:** `sahyadri:YOUR_WALLET_ADDRESS.WORKERNAME`
...
```powershell
$env:RUST_LOG="info,sahyadri_stratum_bridge=debug"
```
